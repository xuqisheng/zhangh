...
DROP PROCEDURE IF EXISTS `up_ihotel_audit_rep_jiedai_tax`$$

CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_audit_rep_jiedai_tax`(
...
BEGIN
	...
	DECLARE var_charge_net		DECIMAL(12,2) DEFAULT 0;
	...
	DECLARE var_amount_tax		DECIMAL(12,2) DEFAULT 0.00;
	...
	DECLARE c_gltemp_1 CURSOR FOR 
		SELECT a.accnt_type,a.modu_code,a.rmno,a.ta_code,b.arrange_code,a.charge,a.charge_net,a.charge_tax,a.charge_oth,a.accnt,a.trans_accnt,a.quantity,a.market,a.charge_srv,a.package_rate,a.pay,a.act_flag,b.cat_posting FROM temp_account_2 a LEFT JOIN code_transaction b ON a.ta_code = b.code AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id;
	DECLARE p_deptjie CURSOR FOR 
		SELECT a.accnt_type,a.modu_code,a.id,a.ta_code,a.market,a.source,a.charge,a.charge_net,a.charge_tax FROM tmp_deptjie a;
	...
	DECLARE c_vipjie CURSOR FOR 
		SELECT a.accnt_type,a.modu_code,a.id,a.ta_code,a.market,a.source,a.charge,a.charge_net,a.charge_tax FROM tmp_vipjie a;
	...
	DECLARE c_charge_ent CURSOR FOR 
		SELECT a.accnt_type,'08',a.accnt,a.ta_code1,a.market,a.source,a.amount,a.amount_tax FROM tmp_apportion a ;
	...
	DECLARE c_jie_custor CURSOR FOR 
		SELECT a.toop,a.toclass,a.day01,a.day02,a.day03,a.day04,a.day05,a.day06,a.day07,a.day08,a.day09,a.day10,a.day11,a.day12,a.day13,a.day14,a.day15,a.day99 FROM rep_jie_hd_tax a WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND a.rectype = 'B';
	...
	CREATE TEMPORARY TABLE temp_account_2(
		...
		charge_net	DECIMAL(12,2) NOT NULL DEFAULT 0.00,
	...
	INSERT INTO temp_account_2(accnt_type,modu_code,rmno,ta_code,arrange_code,charge,charge_net,charge_tax,charge_oth,accnt,quantity,market,charge_srv,package_rate,pay,act_flag,trans_flag,trans_accnt)
		SELECT a.accnt_type,a.modu_code,a.rmno,a.ta_code,a.arrange_code,a.charge,a.charge-a.charge_tax,a.charge_tax,a.charge_oth,a.accnt,a.quantity,a.market,a.charge_srv,a.package_rate,a.pay,a.act_flag,a.trans_flag,a.trans_accnt
	...
	INSERT INTO tmp_deptjie(accnt_type,modu_code,id,ta_code,market,source,charge,charge_net,charge_tax)
		SELECT 'POS','04',a.id,a.code,a.market,a.source,a.amount_day,a.amount_day-a.amount_tax,a.amount_tax FROM pos_deptjie_hd a WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND a.biz_date = var_bdate;
	...
	INSERT INTO tmp_apportion(accnt_type,accnt,ta_code1,descript1,ta_code2,descript2,amount,amount_tax)
		SELECT biz_type,accnt1,ta_code1,ta_descript1,ta_code2,ta_descript2,apportion_amount-apportion_tax,apportion_tax FROM apportion_detail 
	...
	-- 会员
 	INSERT INTO tmp_vipjie(accnt_type,modu_code,id,ta_code,market,source,charge,charge_net,charge_tax)
		SELECT 'VIP','02',a.card_id,a.ta_code,'','',a.charge,a.charge-TRUNCATE(a.charge/(1+b.commission)*b.commission,2),TRUNCATE(a.charge/(1+b.commission)*b.commission,2) FROM card_account a,code_transaction b WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND a.biz_date = var_bdate AND a.source = 'OWN'
			AND a.ta_code=b.code AND b.hotel_id=arg_hotel_id AND b.hotel_group_id=arg_hotel_group_id;
	...
	DELETE FROM rep_jie_hd_tax WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id;
	INSERT INTO rep_jie_hd_tax(hotel_group_id,hotel_id,biz_date,orderno,itemno,modeno,classno,descript,descript1,rectype,toop,toclass,sequence)
	...
	UPDATE rep_jie_hd_tax SET day01=0,day02=0,day03=0,day04=0,day05=0,day06=0,day07=0,day08=0,day09=0,day10=0,
	...
	UPDATE rep_jie_hd_tax SET modeno = REPLACE(modeno,':',',') WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id;
 	UPDATE rep_jie_hd_tax SET modeno = REPLACE(modeno,'GCNULL,','') WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id;
	UPDATE rep_jie_hd_tax SET modeno = REPLACE(modeno,';',',') WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id;
 	UPDATE rep_jie_hd_tax SET modeno = CONCAT(',',modeno) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND modeno <> '';
	DELETE FROM rep_dai_hd_tax WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id;
	INSERT INTO rep_dai_hd_tax(hotel_group_id,hotel_id,biz_date,orderno,itemno,modeno,classno,descript,descript1,sequence)
	...
	UPDATE rep_dai_hd_tax SET credit01=0,credit02=0,credit03=0,credit04=0,credit05=0,credit06=0,credit07=0,credit08=0,credit09=0,credit10=0,credit11=0,credit12=0,credit13=0,credit14=0,sumcre=0,debit=0,credit=0
		WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id;
  	-- 一、处理前台和AR
	OPEN c_gltemp_1;
	SET done_cursor = 0;
	FETCH c_gltemp_1 INTO var_accnt_type,var_modu_code,var_rmno,var_tacode,var_arrange_code,var_charge,var_charge_net,var_charge_tax,var_charge_oth,var_accnt,var_trans_accnt,var_quantity,var_market,var_charge_srv,var_package_rate,var_pay,var_act_flag,var_cat_posting;
	WHILE done_cursor = 0	DO
		BEGIN
			IF var_arrange_code = '98' THEN
				CALL up_ihotel_audit_rep_jiedai_dai_tax(arg_hotel_group_id,arg_hotel_id,var_accnt_type,'02',var_tacode,var_pay);		
			END IF;
			CALL up_ihotel_audit_rep_jiedai_jiedai_tax(arg_hotel_group_id,arg_hotel_id,var_accnt_type,var_accnt,var_charge,var_pay);
			
			IF var_tacode <> '' THEN
				CALL up_ihotel_audit_rep_jiedai_jie_tax(arg_hotel_group_id,arg_hotel_id,var_accnt_type,var_modu_code,var_accnt,var_tacode,var_market,'',var_charge_net,var_charge_tax,var_trans_accnt);	
			END IF;
	...
	CLOSE c_gltemp_1 ;			
 	-- 二、处理餐饮借方
  	OPEN p_deptjie;
	SET done_cursor = 0;
	FETCH p_deptjie INTO var_accnt_type,var_modu_code,var_pos_id,var_tacode,var_market,var_source,var_charge,var_charge_net,var_charge_tax;
	WHILE done_cursor = 0	DO
		BEGIN
		 	CALL up_ihotel_audit_rep_jiedai_jie_tax(arg_hotel_group_id,arg_hotel_id,var_accnt_type,var_modu_code,var_pos_id,var_tacode,var_market,var_source,var_charge_net,var_charge_tax,NULL);
	...
	WHILE done_cursor = 0	DO
		BEGIN
			CALL up_ihotel_audit_rep_jiedai_dai_tax(arg_hotel_group_id,arg_hotel_id,var_accnt_type,'04',var_tacode,var_credit); 
  			SET done_cursor = 0;
			FETCH p_deptdai INTO var_accnt_type,var_tacode,var_descript,var_credit;
		END;
	END WHILE;	
	...
	-- 四、处理会员借方
 	OPEN c_vipjie;
	SET done_cursor = 0;
	FETCH c_vipjie INTO var_accnt_type,var_modu_code,var_vip_id,var_tacode,var_market,var_source,var_charge,var_charge_net,var_charge_tax;
	WHILE done_cursor = 0	DO
		BEGIN
			CALL up_ihotel_audit_rep_jiedai_jie_tax(arg_hotel_group_id,arg_hotel_id,var_accnt_type,var_modu_code,var_accnt,var_tacode,var_market,var_source,var_charge_net,var_charge_tax,NULL);
	...
	WHILE done_cursor = 0	DO
		BEGIN
			CALL up_ihotel_audit_rep_jiedai_dai_tax(arg_hotel_group_id,arg_hotel_id,var_accnt_type,'02',var_tacode,var_credit); 
  			SET done_cursor = 0;
			FETCH c_vipdai INTO var_accnt_type,var_tacode,var_descript,var_credit;
		END;
	END WHILE;	
	...
	IF EXISTS(SELECT 1 FROM sys_option WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND catalog='audit' AND item='ent_audit_show' AND set_value='T') THEN
		...
		OPEN c_charge_ent;
		SET done_cursor = 0;
		FETCH c_charge_ent INTO var_accnt_type,var_modu_code,var_accnt,var_tacode,var_market,var_source,var_amount,var_amount_tax;
		WHILE done_cursor = 0	DO
			BEGIN
				CALL up_ihotel_audit_rep_jiedai_jie_tax(arg_hotel_group_id,arg_hotel_id,var_accnt_type,var_modu_code,var_accnt,var_tacode,var_market,var_source,var_amount,var_amount_tax,NULL);			
				SET done_cursor = 0;
				FETCH c_charge_ent INTO var_accnt_type,var_modu_code,var_accnt,var_tacode,var_market,var_source,var_amount,var_amount_tax;
			END;
		END WHILE;	
		...
		WHILE done_cursor = 0	DO
			BEGIN
				CALL up_ihotel_audit_rep_jiedai_dai_tax(arg_hotel_group_id,arg_hotel_id,var_accnt_type,'08',var_tacode,var_amount); 
				SET done_cursor = 0;
				FETCH c_pay_ent INTO var_accnt_type,var_tacode,var_descript,var_amount;
			END;
		END WHILE;	
	...
	WHILE done_cursor = 0 DO
		BEGIN
			WHILE var_toclass <> SPACE(8) DO
				BEGIN
					IF var_toop = '+' THEN
						UPDATE rep_jie_hd_tax SET day01 = day01+var_day01,day02=day02+var_day02,day03 = day03+var_day03,day04=day04+var_day04,day05=day05+var_day05,day06=day06+var_day06,
					...
					END IF;
  					SELECT toclass,toop INTO var_toclass,var_toop FROM rep_jie_hd_tax WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND classno = var_toclass;
	...
	END WHILE;	
	CLOSE c_jie_custor ;	 
	UPDATE rep_jie_hd_tax SET biz_date = var_bdate WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id;
	UPDATE rep_dai_hd_tax a,rep_dai_hd_tax_history b SET a.last_bl = b.till_bl WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id
			AND a.classno = b.classno AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND b.biz_date = var_bfdate;
	UPDATE rep_dai_hd_tax SET till_bl = IFNULL(last_bl,0) + debit - credit, biz_date = var_bdate WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id;
	-- 会员卡部分
	UPDATE rep_dai_hd_tax a,rep_dai b SET a.last_bl = b.last_bl,a.debit=b.debit,a.credit=b.credit,a.till_bl=b.till_bl,a.sumcre=b.sumcre WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id
 	...
 	SELECT IFNULL(SUM(sumcre),0) INTO var_amount_sumcre FROM rep_dai_hd_tax WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id  AND classno IN('01010','02000','03000','06000');
	...
	UPDATE rep_dai_hd_tax SET sumcre = IFNULL(var_amount_sumcre,0)+IFNULL(var_amount_sumcre_pos,0) 
	...
	END IF;	
	-- 前台人数 | 市场 来源
	UPDATE rep_jie_hd_tax SET day99 = (SELECT IFNULL(SUM(people_total),0) FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='MARKET') WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	UPDATE rep_jie_hd_tax SET day01 = (SELECT IFNULL(SUM(people_total),0) FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='MARKET' AND CODE IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'JZ')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	UPDATE rep_jie_hd_tax SET day02 = (SELECT IFNULL(SUM(people_total),0) FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='MARKET' AND CODE IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'HW')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	UPDATE rep_jie_hd_tax SET day03 = (SELECT IFNULL(SUM(people_total),0) FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='MARKET' AND CODE IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'YX')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	UPDATE rep_jie_hd_tax SET day04 = (SELECT IFNULL(SUM(people_total),0) FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='MARKET' AND CODE IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'SWSK')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	UPDATE rep_jie_hd_tax SET day05 = (SELECT IFNULL(SUM(people_total),0) FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='MARKET' AND CODE IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'LYSK')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	UPDATE rep_jie_hd_tax SET day06 = (SELECT IFNULL(SUM(people_total),0) FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='MARKET' AND CODE IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'TD')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;	
	UPDATE rep_jie_hd_tax SET day07 = (SELECT IFNULL(SUM(people_total),0) FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='MARKET' AND CODE IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'LYZXT')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;	
	UPDATE rep_jie_hd_tax SET day11 = (SELECT IFNULL(SUM(people_total),0) FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='SOURCE' AND CODE = 'XS') WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	UPDATE rep_jie_hd_tax SET day12 = (SELECT IFNULL(SUM(people_total),0) FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='SOURCE' AND CODE = 'XX') WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	-- 前台间夜 | 市场 来源
	...
	UPDATE rep_jie_hd_tax SET day99 = (SELECT IFNULL(SUM(rooms_total),0) FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='MARKET' AND CODE<>'HSE') WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	UPDATE rep_jie_hd_tax SET day01 = (SELECT IFNULL(SUM(rooms_total),0) FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='MARKET' AND CODE IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'JZ' AND CODE<>'HSE')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	UPDATE rep_jie_hd_tax SET day02 = (SELECT IFNULL(SUM(rooms_total),0) FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='MARKET' AND CODE IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'HW' AND CODE<>'HSE')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	UPDATE rep_jie_hd_tax SET day03 = (SELECT IFNULL(SUM(rooms_total),0) FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='MARKET' AND CODE IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'YX' AND CODE<>'HSE')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	UPDATE rep_jie_hd_tax SET day04 = (SELECT IFNULL(SUM(rooms_total),0) FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='MARKET' AND CODE IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'SWSK' AND CODE<>'HSE')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	UPDATE rep_jie_hd_tax SET day05 = (SELECT IFNULL(SUM(rooms_total),0) FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='MARKET' AND CODE IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'LYSK' AND CODE<>'HSE')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	...
	UPDATE rep_jie_hd_tax SET day07 = (SELECT IFNULL(SUM(rooms_total),0) FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='MARKET' AND CODE IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'LYZXT' AND CODE<>'HSE')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;	
	UPDATE rep_jie_hd_tax SET day11 = (SELECT IFNULL(SUM(rooms_total),0) - var_amount_xs FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='SOURCE' AND CODE = 'XS') WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	UPDATE rep_jie_hd_tax SET day12 = (SELECT IFNULL(SUM(rooms_total),0) - var_amount_xx FROM rep_revenue_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND code_type='SOURCE' AND CODE = 'XX') WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	IF EXISTS (SELECT 1 FROM hotel WHERE hotel_group_id = arg_hotel_group_id AND id = arg_hotel_id AND brand_code IN ( 'B','E')) THEN
		...
		UPDATE rep_jie_hd_tax SET day99 = (SELECT IFNULL(SUM(rooms_avl),0) FROM rep_rmsale WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND rep_type='B') WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;		
	END IF;
	
	IF EXISTS (SELECT 1 FROM hotel WHERE hotel_group_id = arg_hotel_group_id AND id = arg_hotel_id AND brand_code IN ( 'B','E')) THEN
		SET var_jiecode = '101040';
		UPDATE rep_jie_hd_tax SET day99 = (SELECT IFNULL(ROUND(SUM(sold_fit+sold_grp+sold_long+sold_ent)*100/SUM(rooms_total),2),0) 
	...
	IF arg_hotel_id = 11 THEN
		UPDATE rep_jie_hd_tax SET day99 = (SELECT IFNULL(SUM(gsts),0) FROM pos_master_history WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND sta<>'X' AND shift NOT IN('1') AND pccode <> '100') WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
		UPDATE rep_jie_hd_tax SET day01 = (SELECT IFNULL(SUM(gsts),0) FROM pos_master_history WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND sta<>'X' AND shift NOT IN('1') AND market IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'JZ' AND pccode <> '100')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
		UPDATE rep_jie_hd_tax SET day02 = (SELECT IFNULL(SUM(gsts),0) FROM pos_master_history WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND sta<>'X' AND shift NOT IN('1') AND market IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'HW' AND pccode <> '100')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
		UPDATE rep_jie_hd_tax SET day03 = (SELECT IFNULL(SUM(gsts),0) FROM pos_master_history WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND sta<>'X' AND shift NOT IN('1') AND market IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'YX' AND pccode <> '100')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
		UPDATE rep_jie_hd_tax SET day04 = (SELECT IFNULL(SUM(gsts),0) FROM pos_master_history WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND sta<>'X' AND shift NOT IN('1') AND market IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'SWSK' AND pccode <> '100')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
		UPDATE rep_jie_hd_tax SET day05 = (SELECT IFNULL(SUM(gsts),0) FROM pos_master_history WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND sta<>'X' AND shift NOT IN('1') AND market IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'LYSK' AND pccode <> '100')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
		...
		UPDATE rep_jie_hd_tax SET day07 = (SELECT IFNULL(SUM(gsts),0) FROM pos_master_history WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND sta<>'X' AND shift NOT IN('1') AND market IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'LYZXT' AND pccode <> '100')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;	
		UPDATE rep_jie_hd_tax SET day11 = (SELECT IFNULL(SUM(gsts),0) FROM pos_master_history WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND sta<>'X' AND shift NOT IN('1') AND source = 'XS'  AND pccode <> '100') WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
		UPDATE rep_jie_hd_tax SET day12 = (SELECT IFNULL(SUM(gsts),0) FROM pos_master_history WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND sta<>'X' AND shift NOT IN('1') AND source = 'XX'  AND pccode <> '100') WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	ELSE
	...
	ELSE
		UPDATE rep_jie_hd_tax SET day99 = (SELECT IFNULL(SUM(gsts),0) FROM pos_master_history WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND sta<>'X' AND shift='1') WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
		UPDATE rep_jie_hd_tax SET day01 = (SELECT IFNULL(SUM(gsts),0) FROM pos_master_history WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND sta<>'X' AND shift='1' AND market IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'JZ')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
		UPDATE rep_jie_hd_tax SET day02 = (SELECT IFNULL(SUM(gsts),0) FROM pos_master_history WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND sta<>'X' AND shift='1' AND market IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'HW')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
		UPDATE rep_jie_hd_tax SET day03 = (SELECT IFNULL(SUM(gsts),0) FROM pos_master_history WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND sta<>'X' AND shift='1' AND market IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'YX')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
		UPDATE rep_jie_hd_tax SET day04 = (SELECT IFNULL(SUM(gsts),0) FROM pos_master_history WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND sta<>'X' AND shift='1' AND market IN (SELECT CODE FROM code_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND parent_code ='market_code' AND code_category = 'SWSK')) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate AND classno = var_jiecode;
	...
	DELETE FROM rep_jie_hd_tax_history WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate;
	INSERT INTO rep_jie_hd_tax_history SELECT * FROM rep_jie_hd_tax WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id;
	DELETE FROM rep_dai_hd_tax_history WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND biz_date = var_bdate;
	INSERT INTO rep_dai_hd_tax_history SELECT * FROM rep_dai_hd_tax WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id;
