...
DROP PROCEDURE IF EXISTS `up_ihotel_audit_guest_internal`$$

CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_audit_guest_internal`(
...
BEGIN
	...
	DECLARE c_cursor CURSOR FOR SELECT id,nation,biz_date,division,id_no,grp_accnt FROM temp_internal;	
	...
	IF NOT EXISTS (SELECT 1 FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'master_base' AND COLUMN_NAME = 'group_code') THEN	
		INSERT INTO temp_internal(id,nation,biz_date,division,id_no,grp_accnt)
	...
	IF NOT EXISTS (SELECT 1 FROM guest_sta_inland WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND guest_class = var_heji) THEN
		INSERT guest_sta_inland (hotel_group_id,hotel_id,DATE, guest_class, list_order, descript, descript1) VALUES (arg_hotel_group_id,arg_hotel_id,var_bdate, var_heji, 0, '合  计', 'Total');
	END IF;
	IF NOT EXISTS (SELECT 1 FROM guest_sta_inland WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND guest_class = var_jingnei) THEN
		INSERT guest_sta_inland (hotel_group_id,hotel_id,DATE, guest_class, list_order, descript, descript1) VALUES (arg_hotel_group_id,arg_hotel_id,var_bdate, var_jingnei, 0, '境  内','Nationals');
	END IF;
	IF NOT EXISTS (SELECT 1 FROM guest_sta_inland WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND guest_class = var_jingwai) THEN
		INSERT guest_sta_inland(hotel_group_id,hotel_id,DATE, guest_class, list_order, descript, descript1) VALUES (arg_hotel_group_id,arg_hotel_id,var_bdate, var_jingwai, 0, '境  外', 'Overseas');
	END IF;
	IF NOT EXISTS (SELECT 1 FROM guest_sta_inland WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND guest_class = var_shengnei) THEN
		INSERT guest_sta_inland(hotel_group_id,hotel_id,DATE, guest_class, list_order, descript, descript1) VALUES (arg_hotel_group_id,arg_hotel_id,var_bdate, var_shengnei, 0, '省  内', 'In province');
	END IF;
	IF NOT EXISTS (SELECT 1 FROM guest_sta_inland WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND guest_class = var_shengwai) THEN
		INSERT guest_sta_inland(hotel_group_id,hotel_id,DATE, guest_class, list_order, descript, descript1) VALUES (arg_hotel_group_id,arg_hotel_id,var_bdate, var_shengwai, 0, '省  外', 'Out province');
	END IF;
	IF NOT EXISTS (SELECT 1 FROM guest_sta_inland WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND guest_class = var_buxiang) THEN
		INSERT guest_sta_inland(hotel_group_id,hotel_id,DATE, guest_class, list_order, descript, descript1) VALUES (arg_hotel_group_id,arg_hotel_id,var_bdate, var_buxiang, 0, '不  详', 'Unknow');
	END IF;	
	
	IF EXISTS(SELECT 1 FROM guest_sta_inland WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND DATE = var_bdate) THEN
	...
	END IF;	
	UPDATE guest_sta_inland SET dtc = 0, dtt = 0, dgc = 0, dgt = 0, dmc = 0, dmt = 0, DATE = var_bdate WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id;
	...
	FETCH c_cursor INTO var_accnt,var_nation,var_arr,var_division,var_idno,var_grpaccnt;
		WHILE done_cursor = 0 DO
			BEGIN
				...
				IF NOT EXISTS(SELECT 1 FROM code_division WHERE hotel_group_id=arg_hotel_group_id AND hotel_id=arg_hotel_id AND code=var_division AND country='CN') THEN
					SET var_division = '';
				END IF;

				SET var_province = '';
				SELECT IFNULL(province,'') INTO var_province FROM code_division WHERE hotel_group_id=arg_hotel_group_id AND hotel_id=arg_hotel_id AND code=var_division AND country='CN';
								...
								BEGIN
									...
									IF NOT EXISTS(SELECT 1 FROM guest_sta_inland WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND guest_class = var_shengwai AND where_from = SUBSTRING(var_division,1,2)) THEN
										...
										SELECT arg_hotel_group_id,arg_hotel_id,var_bdate,var_shengwai,SUBSTRING(var_division,1,2),'','' 
											FROM code_division WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND code = CONCAT(SUBSTRING(var_division,1,2),'0000') AND country='CN';
										...
										UPDATE guest_sta_inland a,code_division b SET a.descript = b.descript,a.descript1 = b.descript_en 
											WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id  AND a.hotel_group_id = b.hotel_group_id AND a.hotel_id = b.hotel_id
												AND a.guest_class = var_shengwai AND CONCAT(a.where_from,'0000') = b.code AND a.where_from <> '' AND b.country='CN';
									END IF;
								...
								BEGIN
									...
									IF NOT EXISTS(SELECT 1 FROM guest_sta_inland WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND guest_class = var_shengnei AND where_from = var_division) THEN
										...
										SELECT arg_hotel_group_id,arg_hotel_id,var_bdate,var_shengnei,var_division,descript,descript_en 
											FROM code_division WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND code = var_division AND province=var_lclpro AND country='CN';
									END IF;									
							...
							ELSEIF var_province <> var_lclpro THEN
								BEGIN
									...
									IF NOT EXISTS(SELECT 1 FROM guest_sta_inland WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND guest_class = var_shengwai AND where_from = SUBSTRING(var_division,1,2)) THEN
										INSERT guest_sta_inland (hotel_group_id,hotel_id,DATE, guest_class, where_from, descript, descript1) 
										SELECT arg_hotel_group_id,arg_hotel_id,var_bdate,var_shengwai,SUBSTRING(var_division,1,2),'','' 
											FROM code_division a WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND a.code = CONCAT(SUBSTRING(var_division,1,2),'0000') AND a.country='CN';
								...
								BEGIN
									...
									IF NOT EXISTS(SELECT 1 FROM guest_sta_inland WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND guest_class = var_shengnei AND where_from = var_division) THEN
										...
										SELECT arg_hotel_group_id,arg_hotel_id,var_bdate,var_shengnei,var_division,a.descript,a.descript_en 
											FROM code_division a WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND a.code = var_division AND a.province=var_lclpro AND a.country='CN';
									END IF;									
	...
	DELETE FROM guest_sta_inland_history WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND DATE = var_bdate ;
	INSERT INTO guest_sta_inland_history SELECT * FROM guest_sta_inland WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND DATE = var_bdate;
