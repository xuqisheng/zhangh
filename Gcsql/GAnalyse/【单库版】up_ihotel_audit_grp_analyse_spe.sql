		...
		BEGIN		
			IF arg_type = 'H' THEN
				BEGIN
					...
					INSERT INTO grp_sales_special (hotel_group_id,hotel_id,year,date_type,date_code,date_desc,date_short,type,class,classdesc,section,sectiondesc,income,nights,persons)		
					SELECT hotel_group_id,hotel_id,var_year,var_datetype,var_datecode,var_datedesc,var_extra1,type,class,classdesc,section,sectiondesc,IFNULL(SUM(income),0),IFNULL(SUM(nights),0),IFNULL(SUM(persons),0)
			...
			IF arg_type = 'G' THEN
				BEGIN
					...
					IF EXISTS(SELECT 1 FROM grp_sales_special WHERE hotel_group_id = arg_hotel_group_id AND year = var_year AND date_type=var_datetype AND date_code=var_datecode) THEN
						INSERT INTO grp_sales_special (hotel_group_id,hotel_id,year,date_type,date_code,date_desc,date_short,type,class,classdesc,section,sectiondesc,income,nights,persons)		
						SELECT arg_hotel_group_id,arg_hotel_id,year,date_type,date_code,date_desc,date_short,type,class,classdesc,section,sectiondesc,IFNULL(SUM(income),0),IFNULL(SUM(nights),0),IFNULL(SUM(persons),0)
						FROM grp_sales_special WHERE hotel_group_id = arg_hotel_group_id AND year = var_year AND date_type=var_datetype AND date_code=var_datecode GROUP BY type,class,section;					
					END IF;	
					-- 协议单位数量分析表(特殊日期) grp_company_special
					DELETE FROM grp_company_special WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND year = var_year AND date_type=var_datetype AND date_code=var_datecode; 				
