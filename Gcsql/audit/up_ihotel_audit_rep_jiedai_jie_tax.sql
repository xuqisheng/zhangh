...
DROP PROCEDURE IF EXISTS `up_ihotel_audit_rep_jiedai_jie_tax`$$

CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_audit_rep_jiedai_jie_tax`(
...
BEGIN
	...
	IF (arg_accnt_type = 'FO' OR arg_accnt_type = 'AR' OR arg_accnt_type = 'VIP') AND arg_modu_code = '02' THEN
		BEGIN
			...
			IF INSTR(var_group_mkt1,arg_market) > 0 THEN
				UPDATE rep_jie_hd_tax SET day01 = day01+arg_charge,day09=day09+arg_charge_tax WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND INSTR(modeno,CONCAT(',',arg_tacode,','))>0;
			ELSEIF 	INSTR(var_group_mkt2,arg_market) > 0 THEN
				UPDATE rep_jie_hd_tax SET day02 = day02+arg_charge,day09=day09+arg_charge_tax WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND INSTR(modeno,CONCAT(',',arg_tacode,','))>0;
			ELSEIF 	INSTR(var_group_mkt3,arg_market) > 0 THEN
				UPDATE rep_jie_hd_tax SET day03 = day03+arg_charge,day09=day09+arg_charge_tax WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND INSTR(modeno,CONCAT(',',arg_tacode,','))>0;
			ELSEIF 	INSTR(var_group_mkt4,arg_market) > 0 THEN
				UPDATE rep_jie_hd_tax SET day04 = day04+arg_charge,day09=day09+arg_charge_tax WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND INSTR(modeno,CONCAT(',',arg_tacode,','))>0;
			ELSEIF 	INSTR(var_group_mkt5,arg_market) > 0 THEN
				UPDATE rep_jie_hd_tax SET day05 = day05+arg_charge,day09=day09+arg_charge_tax WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND INSTR(modeno,CONCAT(',',arg_tacode,','))>0;
			ELSEIF 	INSTR(var_group_mkt6,arg_market) > 0 THEN
				UPDATE rep_jie_hd_tax SET day06 = day06+arg_charge,day09=day09+arg_charge_tax WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND INSTR(modeno,CONCAT(',',arg_tacode,','))>0;
			ELSEIF 	INSTR(var_group_mkt7,arg_market) > 0 THEN
				UPDATE rep_jie_hd_tax SET day07 = day07+arg_charge,day09=day09+arg_charge_tax WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND INSTR(modeno,CONCAT(',',arg_tacode,','))>0;
			ELSE
				UPDATE rep_jie_hd_tax SET day04 = day04+arg_charge,day09=day09+arg_charge_tax WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND INSTR(modeno,CONCAT(',',arg_tacode,','))>0;	
			END IF;	
			IF INSTR(var_group_src1,CONCAT(',',var_src,',')) > 0 THEN
				UPDATE rep_jie_hd_tax SET day11 = day11+arg_charge WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND INSTR(modeno,CONCAT(',',arg_tacode,','))>0;
			ELSEIF INSTR(var_group_src2,CONCAT(',',var_src,',')) > 0 THEN
				UPDATE rep_jie_hd_tax SET day12 = day12+arg_charge WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND INSTR(modeno,CONCAT(',',arg_tacode,','))>0;					
			END IF;
			
 			UPDATE rep_jie_hd_tax SET day99 = day99+arg_charge+arg_charge_tax WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND INSTR(modeno,CONCAT(',',arg_tacode,','))>0;					
		END;
	...
	--			VALUES(arg_accnt,arg_market,arg_source,var_channel,arg_tacode,arg_charge,arg_charge_tax);
			...
			UPDATE rep_jie_hd_tax SET day99 = day99-arg_charge-arg_charge_tax WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND INSTR(modeno,CONCAT(',',arg_tacode,','))>0;					
			UPDATE rep_jie_hd_tax SET day01 = day01+arg_charge+arg_charge_tax,day99=day99+arg_charge+arg_charge_tax WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND classno = '998';
		END;	
