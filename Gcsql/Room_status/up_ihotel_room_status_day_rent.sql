DELIMITER $$
SET sql_notes = 0$$
DROP PROCEDURE IF EXISTS `up_ihotel_room_status_day_rent`$$
CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_room_status_day_rent`(
...
BEGIN
	...
	SET arg_result2 = '0';
	SET arg_result3 = '0';
	SET arg_result_table = 'room_status_day_rent_temp';
	...
	FROM master_base a 
	JOIN rsv_src c ON c.accnt = a.id AND c.occ_flag IN ('RF','RG') AND c.arr_date = c.dep_date AND c.arr_date = arg_date AND c.hotel_group_id = arg_hotel_group_id AND c.hotel_id = arg_hotel_id
	...
	FROM master_base a 
	JOIN rsv_src c ON c.accnt = a.id AND c.occ_flag = 'MF' AND c.arr_date = c.dep_date AND c.arr_date = arg_date AND c.hotel_group_id = arg_hotel_group_id AND c.hotel_id = arg_hotel_id
	...
	IF arg_action <> 0 THEN
		CREATE TEMPORARY TABLE IF NOT EXISTS room_status_day_rent_temp (
		...
		TRUNCATE TABLE room_status_day_rent_temp;
	END IF;

	IF arg_action = 1 THEN
		INSERT INTO room_status_day_rent_temp(accnt, sta, grpAccnt, name, vip, occFlag, rmtype, rmno, rmnoSon, realRate, rmnum, peopleNum, isSure, market, src, rsvType, arrDate, depDate, grpComName, ageSouName, rsvNo, rsvId, masterId, rsvClass)
