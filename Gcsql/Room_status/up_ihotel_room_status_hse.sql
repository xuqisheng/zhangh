DELIMITER $$
SET sql_notes = 0$$
DROP PROCEDURE IF EXISTS `up_ihotel_room_status_hse`$$
CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_room_status_hse`(
...
BEGIN
	...
	SET arg_result2 = '0';
	SET arg_result3 = '0';
	SET arg_result_table = 'room_status_hse_temp';
	...
	JOIN code_base f ON f.code = c.market AND f.parent_code = 'market_code' AND f.flag = 'HSE' AND f.hotel_group_id = arg_hotel_group_id AND f.hotel_id = arg_hotel_id
	...
	FROM master_base a 
	JOIN code_base f ON f.code = a.market AND f.parent_code = 'market_code' AND f.flag = 'HSE' AND f.hotel_group_id = arg_hotel_group_id AND f.hotel_id = arg_hotel_id
	WHERE a.is_resrv = 'F' AND a.rsv_class IN ('F','D') AND a.sta IN ('R','I') AND DATE(a.arr) <= arg_date AND DATE(a.dep) > arg_date AND a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id 
	AND (rmtype_amount = 0 OR a.rmtype IN (SELECT code FROM rmtypes_temp));
	
	IF arg_action <> 0 THEN
		CREATE TEMPORARY TABLE IF NOT EXISTS room_status_hse_temp (
		...
		TRUNCATE TABLE room_status_hse_temp;
	END IF;
	
	IF arg_action = 1 THEN
		INSERT INTO room_status_hse_temp(accnt, sta, grpAccnt, name, vip, occFlag, rmtype, rmno, rmnoSon, realRate, rmnum, peopleNum, isSure, market, src, rsvType, arrDate, depDate, grpComName, ageSouName, rsvNo, rsvId, masterId, rsvClass)
