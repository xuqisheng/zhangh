DELIMITER $$
SET sql_notes = 0$$
DROP PROCEDURE IF EXISTS `up_ihotel_room_status_same_day_reservations`$$
CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_room_status_same_day_reservations`(
...
BEGIN
	...
	SET arg_result2 = '0';
	SET arg_result3 = '0';
	SET arg_result_table = 'room_status_same_day_reservations_temp';
	...
	IF arg_date = var_biz_date THEN
		...
		FROM master_base a 
		JOIN rsv_src c ON c.accnt = a.id AND c.occ_flag IN ('RF','RG') AND DATE(c.create_datetime) >= arg_date AND c.hotel_group_id = arg_hotel_group_id AND c.hotel_id = arg_hotel_id
		...
		AND NOT EXISTS(SELECT 1 FROM rsv_src WHERE id = c.id AND hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id) 
		...
		FROM master_base a
		JOIN master_stalog d ON d.id = a.id AND DATE(d.rsv_datetime) >= arg_date AND d.hotel_group_id = arg_hotel_group_id AND d.hotel_id = arg_hotel_id
		WHERE a.is_resrv = 'F' AND a.rsv_class = 'F' AND (a.sta = 'R' OR (a.sta IN ('I','S','O') AND rsv_id <> 0)) AND a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id 
		AND NOT EXISTS(SELECT 1 FROM master_base_till WHERE id = a.id AND hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id) 
		AND (rmtype_amount = 0 OR a.rmtype IN (SELECT code FROM rmtypes_temp));
	END IF;
	
	IF arg_action <> 0 THEN
		CREATE TEMPORARY TABLE IF NOT EXISTS room_status_same_day_reservations_temp (
		...
		TRUNCATE TABLE room_status_same_day_reservations_temp;
	END IF;
	
	IF arg_date = var_biz_date THEN
		IF arg_action = 1 THEN
			INSERT INTO room_status_same_day_reservations_temp(accnt, sta, grpAccnt, name, vip, occFlag, rmtype, rmno, rmnoSon, realRate, rmnum, peopleNum, isSure, market, src, rsvType, arrDate, depDate, grpComName, ageSouName, rsvNo, rsvId, masterId, rsvClass)
