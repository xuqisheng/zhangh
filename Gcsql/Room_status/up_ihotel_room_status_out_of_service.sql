DELIMITER $$
SET sql_notes = 0$$
DROP PROCEDURE IF EXISTS `up_ihotel_room_status_out_of_service`$$
CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_room_status_out_of_service`(
...
BEGIN
	...
	SET arg_result2 = '0';
	SET arg_result3 = '0';
	SET arg_result_table = 'room_status_out_of_service_temp';
	...
	IF arg_action <> 0 THEN
		CREATE TEMPORARY TABLE IF NOT EXISTS room_status_out_of_service_temp (
		...
		TRUNCATE TABLE room_status_out_of_service_temp;
	END IF;
	
	IF arg_action = 1 THEN
		INSERT INTO room_status_out_of_service_temp(rmtype, rmclass, rmno, rmnoSon, sta, building, floor, reasonDesc, osRemark, staTmp, remark)
		...
		LEFT JOIN room_sta_chg d ON d.rmno = c.rmno AND d.bill_sta = 'I' AND d.chg_type = 'OS' AND d.date_begin <= arg_date AND d.date_end > arg_date AND d.hotel_group_id = arg_hotel_group_id AND d.hotel_id = arg_hotel_id 
		LEFT JOIN code_base e ON e.parent_code = 'room_lock_reason' AND e.code = d.reason AND e.hotel_group_id = arg_hotel_group_id AND e.hotel_id = arg_hotel_id 
