DELIMITER $$
SET sql_notes = 0$$
DROP PROCEDURE IF EXISTS `up_ihotel_room_status_extended_stays_rooms`$$
CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_room_status_extended_stays_rooms`(
...
BEGIN
	...
	SET arg_result2 = '0';
	SET arg_result3 = '0';
	SET arg_result_table = 'room_status_extended_stays_rooms_temp';
	...
	IF arg_date = var_biz_date THEN
		...
		FROM master_base a 
		JOIN master_base_till e ON e.id = a.id AND e.sta = 'I' AND DATE(e.dep) < arg_date AND e.hotel_group_id = arg_hotel_group_id AND e.hotel_id = arg_hotel_id
		WHERE a.is_resrv = 'F' AND a.rsv_class = 'F' AND a.sta = 'I' AND DATE(a.dep) > arg_date AND a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id 
		AND (rmtype_amount = 0 OR a.rmtype IN (SELECT code FROM rmtypes_temp));
	END IF;
	
	IF arg_action <> 0 THEN
		CREATE TEMPORARY TABLE IF NOT EXISTS room_status_extended_stays_rooms_temp (
		...
		TRUNCATE TABLE room_status_extended_stays_rooms_temp;
	END IF;
	
	IF arg_date = var_biz_date THEN
		IF arg_action = 1 THEN
			INSERT INTO room_status_extended_stays_rooms_temp(accnt, sta, grpAccnt, name, vip, occFlag, rmtype, rmno, rmnoSon, realRate, rmnum, peopleNum, isSure, market, src, rsvType, arrDate, depDate, grpComName, ageSouName, rsvNo, rsvId, masterId, rsvClass)
