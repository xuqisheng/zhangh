DELIMITER $$
SET sql_notes = 0$$
DROP PROCEDURE IF EXISTS `up_ihotel_room_status_arrival_actual_rooms`$$
CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_room_status_arrival_actual_rooms`(
...
BEGIN
	...
	SET arg_result2 = '0';
	SET arg_result3 = '0';
	SET arg_result_table = 'room_status_arrival_actual_rooms_temp';
	...
	IF arg_action <> 0 THEN
		CREATE TEMPORARY TABLE IF NOT EXISTS room_status_arrival_actual_rooms_temp (
		...
		TRUNCATE TABLE room_status_arrival_actual_rooms_temp;
	END IF;

	IF arg_action = 1 THEN
		INSERT INTO room_status_arrival_actual_rooms_temp(accnt, sta, grpAccnt, name, vip, occFlag, rmtype, rmno, rmnoSon, realRate, rmnum, peopleNum, isSure, market, src, rsvType, arrDate, depDate, grpComName, ageSouName, rsvNo, rsvId, masterId, rsvClass)
