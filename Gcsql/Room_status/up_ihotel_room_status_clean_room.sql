DELIMITER $$
SET sql_notes = 0$$
DROP PROCEDURE IF EXISTS `up_ihotel_room_status_clean_room`$$
CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_room_status_clean_room`(
...
BEGIN
	...
	SET arg_result2 = '0';
	SET arg_result3 = '0';
	SET arg_result_table = 'room_status_clean_room_temp';
	...
	IF arg_date = var_biz_date THEN
		...
		FROM room_no b
		JOIN room_sta c ON c.rmno = b.code AND c.sta = 'R' AND c.hotel_group_id = arg_hotel_group_id AND c.hotel_id = arg_hotel_id 
	...
	IF arg_action <> 0 THEN
		CREATE TEMPORARY TABLE IF NOT EXISTS room_status_clean_room_temp (
		...
		TRUNCATE TABLE room_status_clean_room_temp;
	END IF;
	
	IF arg_date = var_biz_date THEN
		IF arg_action = 1 THEN
			INSERT INTO room_status_clean_room_temp(rmtype, rmclass, rmno, rmnoSon, sta, building, floor, staTmp, remark)
			SELECT b.rmtype,b.rmclass,b.code,'',c.sta,b.building,b.floor,IFNULL(g.descript,IFNULL(c.sta_tmp,'')),IFNULL(f.remark,'')
			FROM room_no b
			JOIN room_sta c ON c.rmno = b.code AND c.sta = 'R' AND c.sta_occ = 'O' AND c.hotel_group_id = arg_hotel_group_id AND c.hotel_id = arg_hotel_id 
		...
		ELSEIF arg_action = 2 THEN
			...
			SELECT b.rmtype,b.rmclass,b.code,'',c.sta,b.building,b.floor,IFNULL(g.descript,IFNULL(c.sta_tmp,'')),IFNULL(f.remark,'')
			FROM room_no b
			JOIN room_sta c ON c.rmno = b.code AND c.sta = 'R' AND c.sta_occ = 'V' AND c.hotel_group_id = arg_hotel_group_id AND c.hotel_id = arg_hotel_id 
			LEFT JOIN room_sta_chg f ON f.rmno = c.rmno AND f.bill_sta = 'I' AND f.chg_type = 'TMP' AND f.hotel_group_id = arg_hotel_group_id AND f.hotel_id = arg_hotel_id 
			LEFT JOIN code_room_tmp g ON g.code = c.sta_tmp AND g.hotel_group_id = arg_hotel_group_id AND g.hotel_id = arg_hotel_id 
			WHERE b.is_villa <> 'B' AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id 
			AND (rmtype_amount = 0 OR b.rmtype IN (SELECT code FROM rmtypes_temp));
