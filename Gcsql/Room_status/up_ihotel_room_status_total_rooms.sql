DELIMITER $$
SET sql_notes = 0$$
DROP PROCEDURE IF EXISTS `up_ihotel_room_status_total_rooms`$$
CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_room_status_total_rooms`(
...
BEGIN
	...
	SELECT SUM(quantity - IFNULL(b.villa_num,0)) INTO arg_result1
	FROM room_type a 
