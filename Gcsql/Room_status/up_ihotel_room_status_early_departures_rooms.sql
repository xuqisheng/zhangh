DELIMITER $$
SET sql_notes = 0$$
DROP PROCEDURE IF EXISTS `up_ihotel_room_status_early_departures_rooms`$$
CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_room_status_early_departures_rooms`(
...
BEGIN
	...
	SET arg_result2 = '0';
	SET arg_result3 = '0';
	SET arg_result_table = 'room_status_early_departures_rooms_temp';
	...
	IF arg_date = var_biz_date THEN
		...
		WHERE a.is_resrv = 'F' AND a.rsv_class IN ('F','D') AND a.sta IN ('S','O') AND a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id 
		AND (rmtype_amount = 0 OR a.rmtype IN (SELECT code FROM rmtypes_temp));
	END IF;
	
	IF arg_action <> 0 THEN
		CREATE TEMPORARY TABLE IF NOT EXISTS room_status_early_departures_rooms_temp (
		...
		TRUNCATE TABLE room_status_early_departures_rooms_temp;
	END IF;
	
	IF arg_date = var_biz_date THEN
		IF arg_action = 1 THEN
			INSERT INTO room_status_early_departures_rooms_temp(accnt, sta, grpAccnt, name, vip, occFlag, rmtype, rmno, rmnoSon, realRate, rmnum, peopleNum, isSure, market, src, rsvType, arrDate, depDate, grpComName, ageSouName, rsvNo, rsvId, masterId, rsvClass)
