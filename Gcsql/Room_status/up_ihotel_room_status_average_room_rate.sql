DELIMITER $$
SET sql_notes = 0$$
DROP PROCEDURE IF EXISTS `up_ihotel_room_status_average_room_rate`$$
CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_room_status_average_room_rate`(
...
BEGIN
	...
	DECLARE rate_sum VARCHAR(20) DEFAULT '0';
	DECLARE rmnum_sum VARCHAR(10) DEFAULT '0';
	...
	IF use_daily_rsv_rate = 'T' THEN
		SELECT IFNULL(SUM(IFNULL(f.real_share_rate,c.real_rate) * c.rmnum),0), IFNULL(SUM(c.rmnum), 0) INTO rate_sum, rmnum_sum
		FROM master_base a 
		...
		SELECT rate_sum + IFNULL(SUM(IFNULL(f.real_share_rate,a.real_rate)),0), rmnum_sum + COUNT(DISTINCT a.master_id, a.rmno) INTO rate_sum, rmnum_sum
		FROM master_base a 
		...
		AND (rmtype_amount = 0 OR a.rmtype IN (SELECT CODE FROM rmtypes_temp));
	ELSE
		SELECT IFNULL(SUM(c.real_rate * c.rmnum),0), IFNULL(SUM(c.rmnum), 0) INTO rate_sum, rmnum_sum
		FROM master_base a 
		JOIN rsv_src c ON c.accnt = a.id AND c.occ_flag IN ('RF','RG') AND c.arr_date <= arg_date AND c.dep_date > arg_date AND c.hotel_group_id = arg_hotel_group_id AND c.hotel_id = arg_hotel_id
		WHERE a.is_resrv = 'T' AND a.rsv_class IN ('F','G') AND a.sta IN ('R','I') AND a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id 
		AND (rmtype_amount = 0 OR a.rmtype IN (SELECT CODE FROM rmtypes_temp));
		
		SELECT rate_sum + IFNULL(SUM(a.real_rate),0), rmnum_sum + COUNT(DISTINCT a.master_id, a.rmno) INTO rate_sum, rmnum_sum
		FROM master_base a 
		WHERE a.is_resrv = 'F' AND a.rsv_class IN ('F','D') AND a.sta IN ('R','I') AND a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id 
		AND (rmtype_amount = 0 OR a.rmtype IN (SELECT CODE FROM rmtypes_temp));
	END IF;

	SET arg_result1 = IFNULL(TRUNCATE(rate_sum / rmnum_sum, 2),0);
