...
DROP PROCEDURE IF EXISTS `ihotel_up_rmrsv_rsv_src_smart`$$

CREATE DEFINER=`root`@`%` PROCEDURE `ihotel_up_rmrsv_rsv_src_smart`(
...
BEGIN
	...
	SELECT a.hotel_group_id,a.hotel_id,IF(b.roomno<>'','MF',(IF((LEFT(b.accnt,1)='G' OR LEFT(b.accnt,1)='M'),'RG','RF'))),a.accnt_new,0,b.type,b.roomno,0,IF(b.blkmark<>'T','F','T'),b.begin_,b.end_,b.quantity,
		...
		FROM up_map_accnt a,migrate_yl.rsvsrc b WHERE a.hotel_group_id=arg_hotel_group_id AND a.hotel_id=arg_hotel_id 
