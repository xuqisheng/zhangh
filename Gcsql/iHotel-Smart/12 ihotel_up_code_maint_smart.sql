...
DROP PROCEDURE IF EXISTS `ihotel_up_code_maint_smart`$$

CREATE DEFINER=`root`@`%` PROCEDURE `ihotel_up_code_maint_smart`(
...
BEGIN
	...
	SELECT a.hotel_group_id,a.hotel_id,a.id,'F',c.number - 1,c.pccode,c.cardno,c.expiry_date,'',c.creditno,
			...
			FROM master_base a,up_map_accnt b,migrate_yl.accredit c
	...
	SELECT a.hotel_group_id,a.hotel_id,a.id,'A',c.number -1 ,c.pccode,c.cardno,c.expiry_date,'',c.creditno,
			c.amount,0,0,'R',c.empno1,c.bdate1,CONVERT(c.shift1,SIGNED),c.log_date1,NULL,NULL,NULL,NULL,
			NULL,NULL,c.empno1,c.log_date1,'F'
			FROM ar_master a,up_map_accnt b,migrate_yl.accredit c
			WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id
			AND a.sta IN ('I','O') AND a.id = b.accnt_new AND b.accnt_type='armst' AND b.accnt_old=c.accnt AND c.tag='0';
	
	UPDATE accredit a,up_map_code b SET a.ta_code=b.code_new WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND b.cat='paymth' AND a.ta_code=b.code_old;
	...
	UPDATE master_base a,up_map_code b SET a.salesman = b.code_new WHERE b.cat = 'saleman' AND a.salesman = b.code_old AND a.salesman<>'' AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND a.hotel_group_id=b.hotel_group_id AND a.hotel_id = 0;
	UPDATE master_base a,up_map_code b SET a.pay_code = b.code_new WHERE a.pay_code = b.code_old AND b.cat='paymth' AND a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id;
	UPDATE master_base SET pay_code = '10001' WHERE pay_code = '' AND hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id;
    UPDATE master_base SET dsc_reason = 'OO' WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND rack_rate <> real_rate AND dsc_reason = '' ;	
	-- 修复打开团队主单账务时报错	
	...
	-- 修复同住
	UPDATE master_base a,up_map_accnt b,migrate_yl.master c,up_map_accnt d SET master_id=d.accnt_new
	...
	-- 修复联房
		UPDATE master_base a,up_map_accnt b,migrate_yl.master c,up_map_accnt d
	...
	UPDATE master_base a,up_map_accnt b,migrate_yl.master c,up_map_accnt d SET a.grp_accnt=d.accnt_new,a.rsv_id=d.accnt_new
	WHERE a.hotel_group_id=arg_hotel_group_id AND a.hotel_id=arg_hotel_id AND b.hotel_group_id=arg_hotel_group_id AND b.hotel_id=arg_hotel_id
	...
	/*
	UPDATE master_base SET up_rmtype = '' 	WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND up_rmtype IS NULL;	
	UPDATE master_base SET up_user = '' 	WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND up_user IS NULL;
	UPDATE master_base SET up_reason = '' 	WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND up_reason IS NULL;
	UPDATE rsv_src 	SET up_rmtype = '' 	WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND up_rmtype IS NULL;
	UPDATE rsv_src 	SET up_user = '' 	WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND up_user IS NULL;
	UPDATE rsv_src 	SET up_reason = '' 	WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND up_reason IS NULL;
	*/
	...
	INSERT INTO profile_card (hotel_group_id,hotel_id,master_type,master_id,card_type,card_no,date_begin,date_end,remark,is_master,is_halt,create_user,create_datetime,modify_user,modify_datetime) 
		SELECT a.hotel_group_id,a.hotel_id,'F',a.accnt_new,b.cardcode,b.cardno,ADDDATE(b.expiry_date,INTERVAL -15 DAY),b.expiry_date,'','F','F',b.cby,b.changed,b.cby,b.changed FROM up_map_accnt a,migrate_yl.guest_card b WHERE a.hotel_group_id=arg_hotel_group_id AND a.hotel_id=arg_hotel_id AND a.accnt_type='GUEST' AND a.accnt_old=b.no GROUP BY a.accnt_new;
	UPDATE profile_card SET card_type='bankCard' WHERE hotel_group_id=arg_hotel_group_id AND hotel_id=arg_hotel_id AND card_type NOT IN ('CJF','JLA','ZAJ','ZZJ');	
	
	/*
	-- 包价对应的消费账账号(注意处理X5中的消费账号)
	UPDATE code_package a,up_map_accnt b SET a.accnt = b.accnt_new WHERE b.accnt_old='C000059' AND b.accnt_type='consume' AND a.code IN ('BF0','BF1','BF2','BF3') AND a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id;
	UPDATE code_package a,up_map_accnt b SET a.accnt = b.accnt_new WHERE b.accnt_old='C000082' AND b.accnt_type='consume' AND a.code IN ('1XT','2XT') AND a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id;	
	UPDATE code_package a,up_map_accnt b SET a.accnt = b.accnt_new WHERE b.accnt_old='C000060' AND b.accnt_type='consume' AND a.code IN ('1BFG','2BFG') AND a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id;	
	UPDATE code_package a,up_map_accnt b SET a.accnt = b.accnt_new WHERE b.accnt_old='C000083' AND b.accnt_type='consume' AND a.code IN ('1HT','2HT') AND a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id;
	UPDATE code_package a,up_map_accnt b SET a.accnt = b.accnt_new WHERE b.accnt_old='C000085' AND b.accnt_type='consume' AND a.code IN ('1ST','2ST') AND a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id;
	*/
	-- 为重建房价准备
