...
DROP PROCEDURE IF EXISTS `ihotel_up_guest_grp_smart`$$

CREATE DEFINER=`root`@`%` PROCEDURE `ihotel_up_guest_grp_smart`(
...
BEGIN
	...
	DECLARE var_mobile 	VARCHAR(15);
	DECLARE var_phone  	VARCHAR(13);
	...
	DECLARE var_comment VARCHAR(250);
	...
	DECLARE c_cursor CURSOR FOR SELECT NO,sta,sno,cno,TRIM(name),IF(nation IN('CN','TW'),TRIM(fname),TRIM(lname)),IF(nation IN('CN','TW'),TRIM(lname),TRIM(fname)),TRIM(name2),TRIM(name3),TRIM(name4),lang,title,salutation,liason,liason1,sex,
		city,birth,occupation,vip,src,market,nation,race,religion,TRIM(street),mobile,phone,fax,wetsite,email,unit,zip,belong,
		...
		zip1,mobile1,phone1,fax1,email1,visaid,visaend,visano,visaunit,rjplace,lawman,regno,bank,bankno,taxno,arr,dep,fv_date,fv_room,fv_rate,lv_date,lv_room,lv_rate,crtby,crttime,
		...
		FROM migrate_yl.guest WHERE class = 'G';
	
	DECLARE CONTINUE HANDLER FOR SQLSTATE '02000' SET done_cursor = 1;
	DELETE FROM up_status WHERE hotel_id=arg_hotel_id AND up_step='guest_grp';
	INSERT INTO up_status(hotel_id,up_step,time_begin,time_end,time_long,remark) VALUES(arg_hotel_id,'guest_grp',NOW(),NULL,0,''); 
		
	OPEN c_cursor;
	SET done_cursor = 0;
	FETCH c_cursor INTO var_no,var_sta,var_sno,var_cno,var_name,var_lname,var_fname,var_name2,var_name3,var_name4,var_lang,var_title,var_salutation,var_liason,var_liason1,var_sex,
	...
	WHILE done_cursor = 0 DO
		...
		INSERT INTO guest_base(hotel_group_id,hotel_id,NAME,last_name,first_name,name2,name3,name_combine,is_save,sex,
			...
			VALUES(arg_hotel_group_id,0,var_name,var_lname,var_fname,var_name2,var_name3,var_name4,'F',var_sex,
				var_lang,var_title,var_salutation,var_birth,var_race,var_religion,'',var_occupation,var_nation,var_idcls,var_ident,NULL,var_cusno,var_unit,
				NULL,NULL,var_comment,arg_hotel_id,var_crtby,var_crttime,arg_hotel_id,var_cby,var_changed);
		...
		IF NOT EXISTS(SELECT 1 FROM guest_link_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = 0 AND id = var_guest_id) THEN
			INSERT INTO guest_link_base (hotel_group_id,hotel_id,id,mobile,phone,fax,email,website,msn,qq,sns,blog,
				...
				VALUES(arg_hotel_group_id,'0',var_guest_id,var_liason1,var_phone,var_fax,var_email,var_wetsite,'','','','',
					var_liason,'',arg_hotel_id,var_crtby,var_crttime,arg_hotel_id,var_cby,var_changed);
		END IF;
		...
		IF NOT EXISTS(SELECT 1 FROM guest_link_addr WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = 0 AND guest_id = var_guest_id) THEN
			INSERT INTO guest_link_addr (hotel_group_id,hotel_id,guest_id,addr_name,addr_type,is_default,country,state,city,division,street,
				...
				VALUES(arg_hotel_group_id,0,var_guest_id,'默认地址','HOME','T',var_country,var_state,var_town,'',var_street,
					var_zip,'0','',arg_hotel_id,var_crtby,var_crttime,arg_hotel_id,var_cby,var_changed);
		END IF;
		-- 酒店档案 
		IF NOT EXISTS(SELECT 1 FROM guest_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND guest_id = var_guest_id) THEN
			INSERT INTO guest_type (hotel_group_id,hotel_id,guest_id,sta,manual_no,sys_cat,flag_cat,grade,latency,
				...
				VALUES(arg_hotel_group_id,arg_hotel_id,var_guest_id,var_sta,var_no,'G','','',var_latency,
					...
					'',var_saleid,var_araccnt1,var_araccnt2,'000000000000000000000000000000','',var_crtby,var_crttime,var_changed,var_cby,var_changed);
		END IF;
		-- 集团档案 
		IF NOT EXISTS(SELECT 1 FROM guest_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = 0 AND guest_id = var_guest_id) THEN
			INSERT INTO guest_type (hotel_group_id,hotel_id,guest_id,sta,manual_no,sys_cat,flag_cat,grade,latency,
					...
					class1,class2,class3,class4,code1,code2,code3,code4,code5,
					flag,saleman,ar_no1,ar_no2,extra_flag,extra_info,comments,create_user,create_datetime,modify_user,modify_datetime) 
				VALUES(arg_hotel_group_id,'0',var_guest_id,var_sta,var_no,'G','','',var_latency,
					var_src,var_market,var_vip,var_belong,'',var_cardno,'','F',var_arr,var_dep,
					var_class1,var_class2,var_class3,var_class4,var_code1,var_code2,var_code3,var_code4,var_code5,
					'',var_saleid,var_araccnt1,var_araccnt2,'000000000000000000000000000000',var_extrainf,var_remark,var_crtby,var_crttime,var_cby,var_changed);
		END IF;		
		
		-- 客人喜好
		IF (var_refer1 <> '' OR var_refer2 <> '' OR var_refer3 <> '') AND NOT EXISTS(SELECT 1 FROM guest_prefer WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND guest_id = var_guest_id) THEN
		...
		IF var_guest_id IS NOT NULL THEN
			INSERT INTO up_map_accnt(hotel_group_id,hotel_id,accnt_type,accnt_class,accnt_old,accnt_new) VALUES(arg_hotel_group_id,arg_hotel_id,'GUEST_GRP','G',var_no,var_guest_id);
		END IF;
	...
	UPDATE up_status SET time_end=NOW() WHERE hotel_id=arg_hotel_id AND up_step='guest_grp';
	UPDATE up_status SET time_long=TIMESTAMPDIFF(SECOND,time_begin,time_end) WHERE hotel_id=arg_hotel_id AND up_step='guest_grp';
END$$
