...
DROP PROCEDURE IF EXISTS `ihotel_up_fo_account_smart`$$

CREATE DEFINER=`root`@`%` PROCEDURE `ihotel_up_fo_account_smart`(
...
BEGIN
	...
	SELECT b.hotel_group_id,b.hotel_id,b.accnt_new,0,a.number,a.inumber,'02',a.bdate,a.date,a.pccode,
			...
			FROM migrate_yl.account a,up_map_accnt b 
	...
	UPDATE account a,up_map_code b SET a.ta_code = b.code_new WHERE a.hotel_group_id=arg_hotel_group_id AND a.hotel_id=arg_hotel_id AND b.hotel_group_id=arg_hotel_group_id AND b.hotel_id=arg_hotel_id AND b.cat IN ('paymth','pccode') AND b.code_old = a.ta_code;
 	
	-- 更新arrange_code
	UPDATE account a,code_transaction b SET a.arrange_code=b.arrange_code WHERE a.hotel_group_id=arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND b.hotel_group_id=arg_hotel_group_id AND b.hotel_id=arg_hotel_id AND a.ta_code=b.code;
