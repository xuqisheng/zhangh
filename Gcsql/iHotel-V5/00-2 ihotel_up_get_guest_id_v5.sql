...
DROP PROCEDURE IF EXISTS `ihotel_up_get_guest_id_v5`$$

CREATE DEFINER=`root`@`%` PROCEDURE `ihotel_up_get_guest_id_v5`(
	...
	IF arg_guest_id = 0 THEN 
	   BEGIN
		...
		IF var_exists_in_group <> 1 THEN
		   ...
		   -- SELECT MAX(id) INTO arg_guest_id FROM guest_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = 0;
		   ...
		   INSERT INTO guest_link_base (hotel_group_id,hotel_id,id,mobile,phone,fax,email,website,msn,qq,sns,
				  ...
				  VALUES(arg_hotel_group_id,0,arg_guest_id,'','','','','','','','',
		   ...
		   IF arg_address <> '' OR arg_birthplace <> '' THEN			 
				INSERT INTO guest_link_addr (hotel_group_id,hotel_id,guest_id,addr_name,addr_type,is_default,country,
								    state,city,division,street,zipcode,list_order,remark,create_hotel,
								    create_user,create_datetime,modify_hotel,modify_user,modify_datetime) 
							     VALUES(arg_hotel_group_id,'0',arg_guest_id,'默认地址','HOME','T','CN',
								    '','',arg_birthplace,arg_address,'','0','','',
								    'ADMIN',NOW(),'','ADMIN',NOW());
		   END IF;
		   ...
		   INSERT INTO guest_type (hotel_group_id,hotel_id,guest_id,sta,manual_no,sys_cat,flag_cat,grade,latency,class1,class2,class3,class4,
				  ...
				  VALUES(arg_hotel_group_id,'0',arg_guest_id,'I','','F','','','','','','','',
					'','','','1','','','','F',NULL,NULL,'','','','','',
					'','',NULL,NULL,'000000000000000000000000000000','','','ADMIN',NOW(),'ADMIN',NOW());
		END IF;
