...
DROP PROCEDURE IF EXISTS `ihotel_up_company_x5`$$

CREATE DEFINER=`root`@`%` PROCEDURE `ihotel_up_company_x5`(
...
BEGIN
	...
	DELETE FROM portal_pms.up_map_accnt WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND accnt_type = 'COMPANY';
	DELETE FROM portal_pms.up_status WHERE hotel_id=arg_hotel_id AND up_step='COMPANY';
	INSERT INTO portal_pms.up_status(hotel_id,up_step,time_begin,time_end,time_long,remark) VALUES (arg_hotel_id,'COMPANY',NOW(),NULL,0,''); 
   ...
   SET done_cursor = 0;
	...
	WHILE done_cursor = 0 DO		
		...
		-- 喜好
		IF (var_refer1 <> '' OR var_refer2 <> '' OR var_refer3 <> '') AND NOT EXISTS(SELECT 1 FROM company_prefer WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND company_id = var_company_id) THEN
			  INSERT INTO company_prefer (hotel_group_id,hotel_id,company_id,specials,amenity,feature,room_prefer,interest,
				...
				VALUES(arg_hotel_group_id,arg_hotel_id,var_company_id,var_srqs,var_amenity,var_feature,var_rmpref,var_interest,
					var_refer1,var_refer2,var_refer3,var_crtby,var_crttime,var_cby,var_changed);
		END IF;		
		...
		IF var_company_id IS NOT NULL THEN
			INSERT INTO portal_pms.up_map_accnt(hotel_group_id,hotel_id,accnt_type,accnt_class,accnt_old,accnt_new) 
	...
	CLOSE c_cursor;
  	...
  	UPDATE company_type a,portal_pms.up_map_code b SET a.code1 	= b.code_new WHERE a.hotel_group_id=b.hotel_group_id AND a.hotel_id=b.hotel_id AND a.hotel_group_id=arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND b.code_old = a.code1 AND b.code = 'ratecode';
  	UPDATE company_type a,portal_pms.up_map_code b SET a.saleman = b.code_new WHERE a.hotel_group_id=b.hotel_group_id AND a.hotel_id=b.hotel_id AND a.hotel_group_id=arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND b.code_old = a.saleman AND b.code = 'salesman';
	UPDATE company_type a,portal_pms.up_map_code b SET a.src 	= b.code_new WHERE b.hotel_group_id=arg_hotel_group_id AND b.hotel_id=arg_hotel_id AND b.code = 'srccode'  AND b.code_old = a.src  ;
 	UPDATE company_type a,portal_pms.up_map_code b SET a.market = b.code_new WHERE b.hotel_group_id=arg_hotel_group_id AND b.hotel_id=arg_hotel_id AND b.code = 'mktcode'  AND b.code_old = a.market  ;
  	-- UPDATE company_type a,portal_pms.up_map_code b SET a.vip 	= b.code_new WHERE b.hotel_group_id=arg_hotel_group_id AND b.hotel_id=arg_hotel_id AND b.code = 'vip'  AND b.code_old = a.vip  ;
	UPDATE company_base SET name_combine=REPLACE(name_combine,' ','');
	UPDATE company_type SET belong_app_code=1 WHERE hotel_group_id = arg_hotel_group_id;
	...
	-- 从集团库中插回PMS库
	-- 协议公司主单表
	INSERT INTO portal_pms.company_base
		SELECT a.* FROM portal_group.company_base a,portal_pms.up_map_accnt b 
	...
	-- 协议公司关联表
	INSERT INTO portal_pms.company_type
		SELECT a.* FROM portal_group.company_type a,portal_pms.up_map_accnt b 
	...
	INSERT INTO portal_pms.company_production(hotel_group_id,hotel_id,company_id,first_visit_date,first_visit_room,first_visit_rate,
	...
	FROM portal_group.company_production a,portal_pms.up_map_accnt b 
		WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = 0 AND a.company_id = b.accnt_new AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND b.accnt_type = 'COMPANY';
	...
	INSERT INTO portal_pms.profile_extra
		SELECT a.* FROM portal_group.profile_extra a,portal_pms.up_map_accnt b WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND a.master_id = b.accnt_new
			AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND b.accnt_type = 'COMPANY';	
	
	UPDATE portal_pms.up_status SET time_end=NOW() WHERE hotel_id=arg_hotel_id AND up_step='COMPANY';
	UPDATE portal_pms.up_status SET time_long=TIMESTAMPDIFF(SECOND,time_begin,time_end) WHERE hotel_id=arg_hotel_id AND up_step='COMPANY';
