...
DROP PROCEDURE IF EXISTS `up_ihotel_up_guest_grp_yh`$$

CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_up_guest_grp_yh`(
...
BEGIN
	...
	DECLARE c_cursor CURSOR FOR SELECT NO,sta,sno,cno,TRIM(NAME),IF(nation IN('CN','TW'),TRIM(fname),TRIM(lname)),IF(nation IN('CN','TW'),TRIM(lname),TRIM(fname)),TRIM(name2),TRIM(name3),lang,title,salutation,liason,liason1,sex,
		...
		FROM migrate_xmyh.guest WHERE class = 'G';
	...
	WHILE done_cursor = 0 DO
		...
		INSERT INTO guest_base(hotel_group_id,hotel_id,NAME,last_name,first_name,name2,name3,name_combine,is_save,sex,
			...
			VALUES(arg_hotel_group_id,0,var_name,var_lname,var_fname,var_name2,var_name3,CONCAT(var_name,var_name2),'F',var_sex,
				...
				NULL,NULL,var_comment,var_hotel_code,var_crtby,var_crttime,var_hotel_code,var_cby,var_changed);
		...
		IF NOT EXISTS(SELECT 1 FROM guest_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND guest_id = var_guest_id) THEN
			INSERT INTO guest_type (hotel_group_id,hotel_id,guest_id,sta,manual_no,sys_cat,flag_cat,grade,latency,
				...
				VALUES(arg_hotel_group_id,arg_hotel_id,var_guest_id,var_sta,var_no,'G','','',var_latency,
					...
					'',var_saleid,var_araccnt1,var_araccnt2,'000000000000000000000000000000','',CONCAT(var_comment,'--',var_remark),var_crtby,var_changed,var_cby,var_changed);
		END IF;
		-- 集团档案 
		IF NOT EXISTS(SELECT 1 FROM guest_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = 0 AND guest_id = var_guest_id) THEN
			INSERT INTO guest_type (hotel_group_id,hotel_id,guest_id,sta,manual_no,sys_cat,flag_cat,grade,latency,
				...
				VALUES(arg_hotel_group_id,'0',var_guest_id,var_sta,var_no,'G','','',var_latency,
					var_src,var_market,var_vip,var_belong,'',var_cardno,'','F',var_arr,var_dep,
					var_class1,var_class2,var_class3,var_class4,var_code1,var_code2,var_code3,var_code4,var_code5,
					'',var_saleid,var_araccnt1,var_araccnt2,'000000000000000000000000000000',var_extrainf,CONCAT(var_comment,'--',var_remark),var_crtby,var_crttime,var_cby,var_changed);
		END IF;		
		
		-- 客人喜好
		IF (var_refer1 <> '' OR var_refer2 <> '' OR var_refer3 <> '' OR var_srqs <> '' OR var_amenity <> '' OR var_feature <> '') AND NOT EXISTS(SELECT 1 FROM guest_prefer WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND guest_id = var_guest_id) THEN
