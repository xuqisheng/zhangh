...
--   	UPDATE migrate_xmyh.hmaster1 SET m_id=0,l_id=0;
	...
	END IF ; 
	SET var_bigint = var_bigint + 10000;   -- 预留id间隙，在迁移过程中，可能产生的新的id增长空间，完成之后使用./idcheck fix修复
	-- --------------------------------------------------------------------
	...
	*/
	...
	SELECT arg_hotel_group_id,arg_hotel_id,a.logmark,0,'F',a.applname,a.applicant,a.groupno,'',
		...
		'XMYH',a.ref,a.comsg,'','F',a.resby,a.restime,a.cby,a.changed,'F'
	...
	UPDATE master_base_history SET grp_accnt = id WHERE hotel_group_id=arg_hotel_group_id AND hotel_id=arg_hotel_id AND rsv_class = 'G' AND purpose='XMYH';
