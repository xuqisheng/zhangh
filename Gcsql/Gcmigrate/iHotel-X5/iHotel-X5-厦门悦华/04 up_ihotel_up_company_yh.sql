...
DROP PROCEDURE IF EXISTS `up_ihotel_up_company_yh`$$

CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_up_company_yh`(
...
BEGIN
	...
	DECLARE c_cursor CURSOR FOR SELECT NO,sta,class,sno,cno,TRIM(NAME),TRIM(lname),TRIM(fname),TRIM(name2),TRIM(name3),lang,title,salutation,liason,liason1,sex,
		...
		FROM migrate_xmyh.guest WHERE tag = 'B';
	-- 如果按照日期取数，有可能导致当前主单中关联了日期之前的协议单位；
	...
	DELETE FROM up_map_accnt WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND accnt_type = 'COMPANY';
   ...
   SET done_cursor = 0;
	...
	WHILE done_cursor = 0 DO		
		...
		IF NOT EXISTS(SELECT 1 FROM company_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND company_id = var_company_id) THEN
		    INSERT INTO company_type (hotel_group_id,hotel_id,company_id,sta,manual_no,sys_cat,flag_cat,grade,latency,
				...
				VALUES(arg_hotel_group_id,arg_hotel_id,var_company_id,var_sta,IF(var_cno = '',var_no,var_cno),var_class,'','',var_latency,
					var_class1,var_class2,var_class3,var_class4,var_src,var_market,var_vip,var_belong,'',var_cardno,'','F',var_arr,var_dep,
		...
		-- 喜好
		IF (var_refer1 <> '' OR var_refer2 <> '' OR var_refer3 <> '' OR var_srqs <> '' OR var_amenity <> '' OR var_feature <> '') AND NOT EXISTS(SELECT 1 FROM company_prefer WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND company_id = var_company_id) THEN
...
--  	UPDATE company_type a,up_map_code b SET a.code1 = b.code_new WHERE a.hotel_group_id=b.hotel_group_id AND a.hotel_id=b.hotel_id AND a.hotel_group_id=arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND b.code_old = a.code1 AND b.cat = 'ratecode';
  	UPDATE company_type a,up_map_code b SET a.saleman = b.code_new WHERE a.hotel_group_id=b.hotel_group_id AND a.hotel_id=b.hotel_id AND a.hotel_group_id=arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND b.code_old = a.saleman AND b.code = 'salesman';
	UPDATE company_type a,up_map_code b SET a.src = b.code_new WHERE b.hotel_group_id=arg_hotel_group_id AND b.hotel_id=arg_hotel_id AND b.code = 'srccode'  AND b.code_old = a.src  ;
 	UPDATE company_type a,up_map_code b SET a.market = b.code_new WHERE b.hotel_group_id=arg_hotel_group_id AND b.hotel_id=arg_hotel_id AND b.code = 'mktcode'  AND b.code_old = a.market  ;
  	UPDATE company_type a,up_map_code b SET a.vip = b.code_new WHERE b.hotel_group_id=arg_hotel_group_id AND b.hotel_id=arg_hotel_id AND b.code = 'vip'  AND b.code_old = a.vip  ;
--   	UPDATE company_type a,up_map_code b SET a.code1 = b.code_new WHERE b.hotel_group_id=arg_hotel_group_id AND b.hotel_id=arg_hotel_id AND b.cat = 'ratecode'  AND b.code_old = a.code1  ;
	UPDATE guest_base a,up_map_accnt b SET a.company_id = b.accnt_new WHERE b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND b.accnt_type = 'COMPANY' AND a.hotel_group_id = arg_hotel_group_id AND a.company_id = b.accnt_old;
	-- ====================================================================
