...
DROP PROCEDURE IF EXISTS `up_ihotel_up_armst_account_yh`$$

CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_up_armst_account_yh`(
...
BEGIN
	...
	SELECT arg_hotel_group_id,arg_hotel_id,b.id,a.subaccnt,a.number,a.inumber,a.modu_id,a.bdate,a.date,a.pccode,
		...
		a.invoice,a.guestname,a.guestname2,a.shift,a.reason,a.tofrom,'',IF(accntof LIKE 'AR%',CONVERT(CONCAT('14',SUBSTRING(accntof,3)),UNSIGNED),IF(accntof = '','',CONVERT(CONCAT('4',SUBSTRING(accntof,2)),UNSIGNED))),0,
		a.ref,a.ref1,CONCAT(a.ref1,a.accntof),a.ref2,a.roomno,'',0,a.tag,a.audit,NULL,
		NULL,NULL,a.mode1,NULL,NULL,a.empno,a.log_date,a.empno,a.log_date
	FROM migrate_xmyh.ar_detail a,ar_master b WHERE b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND a.accnt = b.arno; 
	...
	-- UPDATE ar_account a,code_transaction b SET a.ta_descript = b.descript,a.ta_descript_en = b.descript_en WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND b.code = a.ta_code;
	...
	SELECT arg_hotel_group_id,arg_hotel_id,IF(a.accnt LIKE 'AR%',b.id,CONVERT(CONCAT(SUBSTRING(a.accntof,2)),UNSIGNED)),a.subaccnt,a.number,a.inumber,a.modu_id,a.bdate,a.date,a.pccode,
		...
		IF(accntof LIKE 'AR%',CONVERT(CONCAT('14',SUBSTRING(accntof,3)),UNSIGNED),IF(accntof = '','',CONVERT(CONCAT('4',SUBSTRING(accntof,2)),UNSIGNED))),0,a.ref,a.ref1,IF(a.ref1 = '','',CONVERT(CONCAT(SUBSTRING(a.ref1,2)),UNSIGNED)),a.ref2,a.roomno,'','',IF(a.billno = '','','B'),
		IF(a.billno = '',0,CONVERT(CONCAT(SUBSTRING(a.billno,2)),UNSIGNED)),'',NULL,NULL,NULL,a.mode1,NULL,a.empno,a.log_date,
		a.empno,a.log_date,b.id,a.ar_subaccnt,a.ar_number,a.ar_inumber,a.ar_tag,a.ar_subtotal,a.ar_pnumber,a.charge9,a.credit9
	FROM migrate_xmyh.ar_account a,ar_master b WHERE b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND a.ar_accnt = b.arno;
	
	-- 补齐房费相关代码市场码，保证AR款待分摊表数据完整(注意写入指定市场码)
	UPDATE ar_detail SET market='RAC' WHERE hotel_group_id=arg_hotel_group_id AND hotel_id=arg_hotel_id AND ar_tag='A' AND ta_code IN ('0001','0002','0003','0004','0005','0006','0007','0008') AND market='';
	...
	-- UPDATE ar_detail a,code_transaction b SET a.ta_descript = b.descript,a.ta_descript_en = b.descript_en WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND b.code = a.ta_code;
