...
DROP PROCEDURE IF EXISTS `up_ihotel_up_master_si_yh`$$

CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_up_master_si_yh`(
...
BEGIN
	-- -----------------------------------------------------------------------
	-- 散客在住、挂账、本日结账数据迁移  migrate_xmyh.master.sta = 'I'/'S'/'O'
	...
	DECLARE c_master CURSOR FOR 
		SELECT a.accnt,b.no,a.type,a.roomno,a.arr,a.dep,a.cusno,a.agent,a.source,a.cardcode,a.cardno,a.saleid,TRIM(b.name),IF(b.nation IN('CN','TW'),TRIM(b.fname),TRIM(b.fname)),IF(b.nation IN('CN','TW'),TRIM(b.lname),TRIM(b.lname)),TRIM(b.name2),TRIM(b.name3),
			...
			a.sta,a.class,a.pcrec,TRIM(a.comsg),IFNULL(a.cby,'ADMIN'),a.restype,a.channel,IFNULL(a.resno,''),IFNULL(a.crsno,''),a.packages,a.gstno,a.children,a.rmnum,a.applname,a.applicant,a.resby,a.restime,a.ciby,a.citime,a.coby,a.cotime,a.depby,a.deptime
		FROM migrate_xmyh.master a,migrate_xmyh.guest b WHERE a.haccnt=b.no AND a.class IN ('F','G','M') AND a.sta IN ('I','O','S') ORDER BY a.accnt;
	...
	DELETE FROM up_map_accnt WHERE hotel_group_id=arg_hotel_group_id AND hotel_id=arg_hotel_id AND accnt_type='master_si' AND accnt_class IN('FI','FS','FO','GI','GS','GO');
	
	OPEN c_master ;
	SET done_cursor = 0;
	
	FETCH c_master INTO var_accnt,var_guestid,var_rmtype,var_rmno,var_arr,var_dep,var_cusno,var_agent,var_source,var_cardcode,var_cardno,var_saleid,var_name,var_lname,var_fname,var_name2,var_name3,
	...
	WHILE done_cursor = 0 DO
		BEGIN
			...
			IF var_accnt = var_last_accnt THEN 				
				...
				VALUES(arg_hotel_group_id,arg_hotel_id,0,'F',var_applname,var_applicant,var_phone,0,var_class,0,0,0,var_biz_date,
					...
					'F',IF(SUBSTRING(var_extra,12,1) = 1,'T','F'),'T','F',IF(SUBSTRING(var_extra,9,1) = 1,'T','F'),SUBSTRING(var_extra,4,1),IF(SUBSTRING(var_extra,5,1) = 1,'T','F'),'0',
			...
			IF var_profile_id > 0 THEN 
				...
				SELECT arg_hotel_group_id,arg_hotel_id,var_id,'GUEST',a.id,var_itimes,a.name,a.last_name,a.first_name,a.name2,a.name3,a.name_combine,
					a.sex,a.language,a.title,a.salutation,'',var_birth,a.race,a.religion,a.career,a.occupation,var_nation,var_nation,'','','',var_address,'',
					d.vip,'','',var_fax,var_email,a.id_code,a.id_no,var_idend,var_visaid,var_visano,NULL,var_visaend,'',var_rjplace,var_rjdate,NULL,
			...
			VALUES(arg_hotel_group_id,arg_hotel_id,var_id,var_resby,var_restime,var_ciby,var_citime,IF(var_sta = 'I','',var_cby),IF(var_sta = 'I',NULL,var_changed),IF(var_sta = 'I','',var_depby),IF(var_sta = 'I',NULL,var_deptime),var_cby,var_changed);
	...
	-- 付款方式
	UPDATE master_base a,up_map_code b SET a.pay_code = b.code_new WHERE a.hotel_group_id=b.hotel_group_id AND a.hotel_id = b.hotel_id AND a.hotel_group_id=arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND b.code = 'paymth' AND b.code_old = a.pay_code AND a.sta <> 'R'; 
	UPDATE master_base a SET a.pay_code = 'CA' WHERE a.hotel_group_id=arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND a.pay_code = ''; 
 	...
 	-- 市场来源
 	UPDATE master_base a,up_map_code b SET a.market = b.code_new WHERE a.hotel_group_id=b.hotel_group_id AND a.hotel_id = b.hotel_id AND a.hotel_group_id=arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND b.code = 'mktcode' AND b.code_old = a.market AND a.sta <> 'R'; 
 	UPDATE master_base a,up_map_code b SET a.src = b.code_new WHERE a.hotel_group_id=b.hotel_group_id AND a.hotel_id = b.hotel_id AND a.hotel_group_id=arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND b.code = 'srccode' AND b.code_old = a.src AND a.sta <> 'R'; 
 	-- 房价码
--   	UPDATE master_base a,up_map_code b SET a.ratecode = b.code_new WHERE a.hotel_group_id=arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND a.hotel_group_id=b.hotel_group_id AND a.hotel_id = b.hotel_id  AND b.code = 'ratecode' AND b.code_old = a.ratecode; 
 	-- 渠道
   	UPDATE master_base a,up_map_code b SET a.channel = b.code_new WHERE a.hotel_group_id=arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND a.hotel_group_id=b.hotel_group_id AND a.hotel_id = b.hotel_id  AND b.code = 'channel' AND b.code_old = a.channel AND a.sta <> 'R'; 
 	-- 预订类型
	...
	-- 销售员
	UPDATE master_base a,up_map_code b SET a.salesman = b.code_new WHERE b.code = 'salesman' AND a.salesman = b.code_old AND a.salesman<>'' AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND a.sta <> 'R';
	-- 国籍国家
...
-- 	UPDATE master_guest a,up_map_code b SET a.nation = b.code_new WHERE a.hotel_group_id=arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND a.hotel_group_id=b.hotel_group_id AND a.hotel_id = b.hotel_id AND b.code = 'nation' AND b.code_old = a.nation; 
 	...
 	-- 保密级别
 	UPDATE master_base a,up_map_code b SET a.is_secret = b.code_new WHERE a.hotel_group_id=arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND a.hotel_group_id=b.hotel_group_id AND a.hotel_id = b.hotel_id AND b.code = 'secret' AND b.code_old = a.is_secret AND a.sta <> 'R'; 
	-- 优惠理由
  	UPDATE master_base a,up_map_code b SET a.dsc_reason = b.code_new WHERE a.hotel_group_id=arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND b.code = 'code_reason' AND b.code_old = a.dsc_reason AND a.sta <> 'R' ;
	-- 升级理由
...
-- 	UPDATE master_base SET amenities = REPLACE(amenities,'PT','PAT') WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND amenities <> '';
-- 	UPDATE master_base SET amenities = REPLACE(amenities,'F2','FL2') WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND amenities <> '';
