...
DROP PROCEDURE IF EXISTS `ihotel_up_master_r_smart`$$

CREATE DEFINER=`root`@`%` PROCEDURE `ihotel_up_master_r_smart`(
...
BEGIN
	...
	DECLARE c_master CURSOR FOR 
		SELECT a.accnt,b.no,a.type,a.roomno,a.rmnum,a.arr,a.dep,a.cusno,a.agent,a.source,a.cardcode,a.cardno,a.saleid,TRIM(b.name),IF(b.nation IN('CN','TW'),TRIM(b.fname),TRIM(b.lname)),IF(b.nation IN('CN','TW'),TRIM(b.lname),TRIM(b.fname)),TRIM(b.name2),TRIM(b.name3),TRIM(b.name4),
			...
			a.sta,a.class,a.pcrec,TRIM(a.comsg),a.cby,a.restype,a.channel,IFNULL(a.resno,''),IFNULL(a.crsno,''),a.packages,a.gstno,a.children,a.applname,a.applicant
		FROM migrate_yl.master a,migrate_yl.guest b WHERE a.haccnt=b.no AND a.class IN ('F','G','M') AND a.sta='R' ORDER BY a.accnt;
	...
	OPEN c_master ;
	SET done_cursor = 0 ;
	FETCH c_master INTO var_accnt,var_guestid,var_rmtype,var_rmno,var_rmnum,var_arr,var_dep,var_cusno,var_agent,var_source,var_cardcode,var_cardno,var_saleid,var_name,var_lname,var_fname,var_name2,var_name3,var_name4,
	...
	WHILE done_cursor = 0 DO
		BEGIN
			...
			IF var_profile_id=0 AND var_ident<>'' THEN 
				...
				CALL ihotel_up_get_guest_id_smart(arg_hotel_group_id,arg_hotel_id,var_haccnt,var_name,var_idcls,var_ident,var_sex,var_birth,var_race,var_address,var_birthplace,var_profile_id); 					
			END IF; 
			...
			IF var_accnt = var_last_accnt THEN 				
				...
				VALUES(arg_hotel_group_id,arg_hotel_id,0,IF(var_rmno<>'','F','T'),var_applname,var_applicant,0,var_class,0,0,0,var_biz_date,
					...
					'F','F','T','F','F','0','F','0',
					var_accnt,'000000000000000000000000000000',0,0,0,0,IFNULL(var_paycode,'10001'),0,'','','',
			...
			IF var_profile_id > 0 THEN 
				...
				SELECT arg_hotel_group_id,arg_hotel_id,var_id,'GUEST',a.id,var_itimes,a.name,a.last_name,a.first_name,a.name2,a.name3,a.name_combine,
					...
					var_vip,var_phone,LEFT(b.mobile,20),var_fax,var_email,a.id_code,a.id_no,a.id_end,var_visaid,'',NULL,var_visaend,'',var_rjplace,NULL,var_rjdate,
