...
DROP PROCEDURE IF EXISTS `ihotel_up_master_ha_smart`$$

CREATE DEFINER=`root`@`%` PROCEDURE `ihotel_up_master_ha_smart`(
...
BEGIN
	...
	DECLARE c_master CURSOR FOR 
		...
		FROM migrate_yl.master a,migrate_yl.guest b WHERE a.haccnt=b.no AND a.class='C' AND a.sta IN ('I','S','O') ORDER BY a.accnt;
