...
DROP PROCEDURE IF EXISTS `ihotel_up_company_smart`$$

CREATE DEFINER=`root`@`%` PROCEDURE `ihotel_up_company_smart`(
...
BEGIN
	...
	DECLARE c_cursor CURSOR FOR SELECT NO,sta,class,sno,cno,TRIM(NAME),TRIM(lname),TRIM(fname),TRIM(name2),TRIM(name3),TRIM(name4),lang,title,salutation,liason,liason1,sex,
		...
		FROM migrate_yl.guest WHERE class IN ( 'A','S','C');
   ...
   OPEN c_cursor;
   SET done_cursor = 0;
	FETCH c_cursor INTO var_no,var_sta,var_class,var_sno,var_cno,var_name,var_lname,var_fname,var_name2,var_name3,var_name4,var_lang,var_title,var_salutation,var_liason,var_liason1,var_sex,
	...
	WHILE done_cursor = 0 DO		
		...
		INSERT INTO company_base (hotel_group_id,hotel_id,name,name2,name3,name_combine,is_save,language,nation,
			...
			VALUES(arg_hotel_group_id,'0',var_name,var_name2,var_name3,var_name4,'F',var_lang,var_nation,
		...
		IF NOT EXISTS(SELECT 1 FROM company_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND company_id = var_company_id) THEN
		    INSERT INTO company_type (hotel_group_id,hotel_id,company_id,sta,manual_no,sys_cat,flag_cat,grade,latency,
				...
				VALUES(arg_hotel_group_id,arg_hotel_id,var_company_id,var_sta,var_cno,var_class,'','',var_latency,
					var_class1,var_class2,var_class3,var_class4,var_src,var_market,var_vip,var_belong,'',var_cardno,'','F',var_arr,var_dep,
					var_code1,var_code2,var_code3,var_code4,var_code5,'',var_saleid,var_araccnt1,var_araccnt2,'000000000000000000000000000000',var_extrainf,var_remark,var_crtby,var_crttime,var_cby,var_changed);
		END IF;
		IF NOT EXISTS(SELECT 1 FROM company_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = '0' AND company_id = var_company_id) THEN
		    INSERT INTO company_type (hotel_group_id,hotel_id,company_id,sta,manual_no,sys_cat,flag_cat,grade,latency,
					class1,class2,class3,class4,src,market,vip,belong_app_code,membership_type,membership_no,membership_level,over_rsvsrc,valid_begin,valid_end,
					code1,code2,code3,code4,code5,flag,saleman,ar_no1,ar_no2,extra_flag,extra_info,comments,create_user,create_datetime,modify_user,modify_datetime) 
				VALUES(arg_hotel_group_id,'0',var_company_id,var_sta,var_cno,var_class,'','',var_latency,
					var_class1,var_class2,var_class3,var_class4,var_src,var_market,var_vip,var_belong,'',var_cardno,'','F',var_arr,var_dep,
					var_code1,var_code2,var_code3,var_code4,var_code5,'',var_saleid,var_araccnt1,var_araccnt2,'000000000000000000000000000000',var_extrainf,var_remark,var_crtby,var_crttime,var_cby,var_changed);
		END IF;
