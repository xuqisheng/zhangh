...
DROP PROCEDURE IF EXISTS `ihotel_up_armst_account_smart`$$

CREATE DEFINER=`root`@`%` PROCEDURE `ihotel_up_armst_account_smart`(
...
BEGIN
	...
	INSERT INTO ar_account(hotel_group_id,hotel_id,accnt,subaccnt,number,inumber,modu_code,biz_date,gen_date,ta_code,
		arrage_code,quantity,charge,pay,balance,charge0,pay0,charge1,pay1,charge9,pay9,balance9,disputed,invoice_code,invoice_amt,
		guest_name,guest_name2,cashier,reason,act_flag,trans_flag,trans_accnt,trans_subaccnt,
	...
	SELECT arg_hotel_group_id,arg_hotel_id,b.id,c.id,a.number,a.inumber,'02',a.bdate,a.log_date,a.pccode,
		'',1,0,0,0,a.charge,a.credit,0,0,0,0,0,0,0,0,
		ref2,ref,shift,'',a.tofrom,0,0,
		IF(a.accntof = '',0,IF(a.accntof LIKE 'AR%',CONVERT(SUBSTRING(a.accntof,3),UNSIGNED),CONVERT(CONCAT(SUBSTRING(a.accntof,2,2),SUBSTRING(a.accntof,5)),UNSIGNED))),
		a.ref,'','','',a.roomno,'',0,'A',1,NULL,
		NULL,NULL,NULL,NULL,NULL,a.empno,a.log_date,a.empno,a.log_date 
	FROM migrate_yl.account a,ar_master b,ar_account_sub c WHERE b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND c.hotel_group_id = arg_hotel_group_id AND c.hotel_id = arg_hotel_id 
	...
	SELECT arg_hotel_group_id,arg_hotel_id,b.id,c.id,a.number,a.inumber,'02',a.bdate,a.log_date,a.pccode,
		'','',1,a.charge,a.charge,0,0,0,0,0,
		0,0,a.credit,(a.charge-a.credit),a.shift,a.crradjt,'','',NULL,NULL,'',a.tofrom,
		IF(a.accntof = '',0,IF(a.accntof LIKE 'AR%',CONVERT(SUBSTRING(a.accntof,3),UNSIGNED),CONVERT(CONCAT(SUBSTRING(a.accntof,2,2),SUBSTRING(a.accntof,5)),UNSIGNED))),0,a.ref,'','',a.ref1,a.roomno,NULL,'','',
		0,'',NULL,NULL,NULL,'',NULL,a.empno,a.log_date,
		a.empno,a.log_date,b.id,c.id,a.number,a.number,'A','F',0,0,0
	FROM migrate_yl.account a,ar_master b,ar_account_sub c WHERE b.hotel_group_id = arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND c.hotel_group_id = arg_hotel_group_id AND c.hotel_id = arg_hotel_id 
	AND c.type = 'SUBACCNT' AND b.id = c.accnt AND a.accnt=b.arno AND a.billno = '';
		
	-- 补齐房费相关代码市场码，保证AR款待分摊表数据完整(注意写入指定市场码)
	UPDATE ar_detail SET market='RAC' WHERE hotel_group_id=arg_hotel_group_id AND hotel_id=arg_hotel_id AND ar_tag='A' AND ta_code IN ('00001','00002','00003','00004','00005','00006','00007','00008') AND market='';
