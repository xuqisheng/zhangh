...
DROP PROCEDURE IF EXISTS `ihotel_up_master_si_smart`$$

CREATE DEFINER=`root`@`%` PROCEDURE `ihotel_up_master_si_smart`(
...
BEGIN
	-- =====================================================================
	-- 散客在住、挂账、本日结账数据迁移  migrate_yl.master.sta = 'I'/'S'/'O'
	...
	DECLARE c_master CURSOR FOR 
		...
		FROM migrate_yl.master a,migrate_yl.guest b WHERE a.haccnt=b.no AND a.class IN ('F','G','M') AND a.sta IN ('I','O','S') ORDER BY a.accnt;
