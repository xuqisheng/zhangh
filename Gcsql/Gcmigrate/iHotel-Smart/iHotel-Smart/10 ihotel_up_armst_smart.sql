...
DROP PROCEDURE IF EXISTS `ihotel_up_armst_smart`$$

CREATE DEFINER=`root`@`%` PROCEDURE `ihotel_up_armst_smart`(
...
BEGIN
	...
	DECLARE c_armst CURSOR FOR SELECT a.accnt,b.name,a.artag1,a.artag2,a.arr,a.dep,a.ref,b.phone,b.fax,b.mobile,a.charge-a.credit,b.sno,a.cby,a.changed 
		FROM migrate_yl.master a,migrate_yl.guest b 
		WHERE a.haccnt=b.no AND a.sta IN ('I','S') AND b.class='R' ORDER BY a.accnt;
	...
	FETCH c_armst INTO var_accnt,var_name,var_artag1,var_artag2,var_arr,var_dep,var_ref,var_phone,var_fax,var_mobile,var_balance,var_sno,var_cby,var_changed; 
	WHILE done_cursor = 0 DO
		BEGIN			
				SET var_profile_id=0,var_profile_type='';
				
				IF var_arr IS NULL THEN 
					SET var_arr = '2013-01-01'; 
				END IF; 
				...
				INSERT INTO ar_master_guest(hotel_group_id,hotel_id,id,name,name2,name_combine,coding,language,nation,
					linkman,address,zipcode,create_user,create_datetime,modify_user,modify_datetime)
				VALUES(arg_hotel_group_id,arg_hotel_id,var_id,var_name,'',var_name,var_accnt,'C','CN',
					'','','',var_cby,var_changed,var_cby,var_changed);
	...
	UPDATE ar_master SET extra_flag = '001000000000000000010000000000' WHERE id IN (SELECT DISTINCT accnt FROM accredit WHERE hotel_group_id=arg_hotel_group_id AND hotel_id=arg_hotel_id AND accnt_type = 'A' ) AND hotel_group_id=arg_hotel_group_id AND hotel_id=arg_hotel_id;
 	...
 	CALL ihotel_up_armst_account_smart(arg_hotel_group_id,arg_hotel_id);
	...
	UPDATE ar_master a,up_map_code b SET a.pay_code = b.code_new WHERE a.hotel_group_id=arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND b.hotel_group_id=arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND b.cat IN ('paymth','pccode') AND b.code_old = a.pay_code; 
