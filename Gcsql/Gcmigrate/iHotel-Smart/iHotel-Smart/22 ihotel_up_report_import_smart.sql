...
DROP PROCEDURE IF EXISTS `ihotel_up_report_import_smart`$$

CREATE DEFINER=`root`@`%` PROCEDURE `ihotel_up_report_import_smart`(
...
BEGIN
	...
	CREATE TABLE migrate_yl.yrmsalerep_bak SELECT * FROM migrate_yl.yrmsalerep_new WHERE 1=2;
	
	INSERT INTO migrate_yl.yrmsalerep_bak
	SELECT DATE,gkey,hall,CODE,descript,SUM(ttl),SUM(mnt),SUM(htl),SUM(avl),SUM(vac),SUM(soldf),SUM(soldg),SUM(soldc),SUM(soldl),SUM(ent),SUM(ext),
		...
		FROM migrate_yl.yrmsalerep_new GROUP BY DATE,gkey,CODE;
		
	UPDATE migrate_yl.yrmsalerep_bak SET hall = 'A' WHERE hall <> '{';
	...
	FROM migrate_yl.yrmsalerep_bak WHERE date >= '2012-01-01' AND CODE NOT LIKE '%{{{%' GROUP BY date,gkey,hall,code;
	...
	SELECT arg_hotel_group_id,arg_hotel_id,DATE,gclass,wfrom,descript,'','',dtc,dgc,0 ,dtt,dgt,0 ,mtc,mgc,0 ,mtt,mgt,0 ,ytc,ygc,0 ,ytt,ygt,0
			FROM migrate_yl.ygststa1 WHERE DATE >='2012-01-01';	
	...
	-- 境外统计报表
	UPDATE migrate_yl.ygststa SET gclass='2',order_='01' WHERE gclass='2' AND order_='' AND nation='' AND descript='省  内';
	UPDATE migrate_yl.ygststa SET gclass='2',order_='02' WHERE gclass='3' AND order_='' AND nation='' AND descript='省  外';
	UPDATE migrate_yl.ygststa SET gclass='3',order_='' WHERE gclass='4' AND order_='' AND nation='' AND descript='---境外---';
	...
	SELECT arg_hotel_group_id,arg_hotel_id,DATE,gclass,nation,order_,descript,'','',dtc,dgc,0 ,dtt,dgt,0 ,mtc,mgc,0 ,mtt,mgt,0 ,ytc,ygc,0 ,ytt,ygt,0
			FROM migrate_yl.ygststa WHERE DATE>='2012-01-01';	
	...
	UPDATE guest_sta_overseas SET list_order='01'
		WHERE hotel_group_id=arg_hotel_group_id AND hotel_id=arg_hotel_id AND descript='省  内' AND guest_class='2' AND nation='';
	UPDATE guest_sta_overseas SET list_order='02',guest_class='2'
		WHERE hotel_group_id=arg_hotel_group_id AND hotel_id=arg_hotel_id AND descript='省  外' AND guest_class='3' AND nation='';
		
	UPDATE guest_sta_overseas_history SET list_order='01'
		WHERE hotel_group_id=arg_hotel_group_id AND hotel_id=arg_hotel_id AND descript='省  内' AND guest_class='2' AND nation='';
	UPDATE guest_sta_overseas_history SET list_order='02',guest_class='2'
		WHERE hotel_group_id=arg_hotel_group_id AND hotel_id=arg_hotel_id AND descript='省  外' AND guest_class='3' AND nation='';	
	
	
	-- 销售分析汇总表
	-- M:market市场码	S:source来源码  C:channel渠道码	R:ratecode房价码 L:restype预订类型	G:gtype大房类
	ALTER TABLE migrate_yl.ymktsummaryrep ADD code_category VARCHAR(30) AFTER code;
	...
	DELETE FROM migrate_yl.ymktsummaryrep WHERE class='G';
	DELETE FROM migrate_yl.ymktsummaryrep WHERE grp='z' AND code='zzz';

	UPDATE migrate_yl.ymktsummaryrep a,code_base b,code_base c SET a.code_category = b.descript 
	...
	UPDATE migrate_yl.ymktsummaryrep a,code_base b SET a.code_category = b.descript 
	...
	UPDATE migrate_yl.ymktsummaryrep a,code_ratecode b SET a.code_category = b.descript 
	...
	SELECT  arg_hotel_group_id,arg_hotel_id,DATE,'MARKET',code_category,code,SUM(tincome),SUM(rincome),SUM(rquan),SUM(pquan),SUM(rarr),SUM(rdep),SUM(noshow),SUM(cxl),SUM(parr),SUM(pdep)
		FROM migrate_yl.ymktsummaryrep WHERE DATE >= '2012-01-01' AND class='M' GROUP BY date,class,code
	UNION ALL
	SELECT  arg_hotel_group_id,arg_hotel_id,DATE,'SOURCE',code_category,'OTH',SUM(tincome),SUM(rincome),SUM(rquan),SUM(pquan),SUM(rarr),SUM(rdep),SUM(noshow),SUM(cxl),SUM(parr),SUM(pdep)
		FROM migrate_yl.ymktsummaryrep WHERE DATE >= '2012-01-01' AND class='S' GROUP BY date,class
	UNION ALL
	SELECT  arg_hotel_group_id,arg_hotel_id,DATE,'CHANNEL',code_category,code,SUM(tincome),SUM(rincome),SUM(rquan),SUM(pquan),SUM(rarr),SUM(rdep),SUM(noshow),SUM(cxl),SUM(parr),SUM(pdep)
		FROM migrate_yl.ymktsummaryrep WHERE DATE >= '2012-01-01' AND class='C' GROUP BY date,class,code		
	UNION ALL
	SELECT  arg_hotel_group_id,arg_hotel_id,DATE,'RATECODE',code_category,code,SUM(tincome),SUM(rincome),SUM(rquan),SUM(pquan),SUM(rarr),SUM(rdep),SUM(noshow),SUM(cxl),SUM(parr),SUM(pdep)
		FROM migrate_yl.ymktsummaryrep WHERE DATE >= '2012-01-01' AND class='R' GROUP BY date,class,code;
	...
	INSERT INTO rep_jour(hotel_group_id,hotel_id,biz_date,code,descript,descript_en,day,month,year,rebate_day,rebate_month,rebate_year,is_show,list_order)
		...
		FROM migrate_yl.yjourrep WHERE DATE >='2012-01-01';
	...
	-- 导入前代码更新
	UPDATE migrate_yl.yjierep SET class = '998' WHERE class = '090' AND mode='E';
	-- 稽核底表 rep_jie	
	...
	SELECT arg_hotel_group_id,arg_hotel_id,DATE,'F',NULL,MODE,class,descript,descript,rectype,toop,toclass,NULL,
		day01,day02,day03,day04,day05,day06,day07,day08,day09,day99,month01,month02,month03,month04,month05,month06,month07,month08,month09,month99
	FROM migrate_yl.yjierep WHERE DATE >= '2012-01-01';	
	...
	SELECT arg_hotel_group_id,arg_hotel_id,DATE,'',itemno,MODE,'01020','   前 厅','  Front Office',sequence,
	 ...
	 FROM migrate_yl.ydairep WHERE DATE >= '2012-01-01' AND class IN ('01020','0130','01998') GROUP BY DATE ORDER BY date,class;	
	...
	SELECT arg_hotel_group_id,arg_hotel_id,DATE,order_,itemno,MODE,'01998',descript,descript1,sequence,
		credit01,credit02,credit03,credit04,credit05,credit06,credit07,sumcre,last_bl,debit,credit,till_bl,
		credit01m,credit02m,credit03m,credit04m,credit05m,credit06m,credit07m,sumcrem,last_blm,debitm,creditm,till_blm
	 FROM migrate_yl.ydairep WHERE DATE >= '2012-01-01' AND class='0120' ORDER BY date,class;
	...
	SELECT arg_hotel_group_id,arg_hotel_id,DATE,order_,itemno,MODE,class,descript,descript,sequence,
		credit01,credit02,credit03,credit04,credit05,credit06,credit07,sumcre,last_bl,debit,credit,till_bl,
		credit01m,credit02m,credit03m,credit04m,credit05m,credit06m,credit07m,sumcrem,last_blm,debitm,creditm,till_blm
	 FROM migrate_yl.ydairep WHERE DATE >= '2012-01-01' AND class IN ('01010','01999','02000','03000','04000','08000','09000') ORDER BY date,class;
	...
	SELECT arg_hotel_group_id,arg_hotel_id,DATE,order_,itemno,MODE,class,descript,descript,NULL,
		last_charge,last_credit,charge,credit,apply,till_charge,till_credit,last_chargem,last_creditm,chargem,creditm,applym,till_chargem,till_creditm
	FROM migrate_yl.yjiedai WHERE DATE >= '2012-01-01' AND class IN('02C','02F') ORDER BY DATE,class;
	...
	FROM migrate_yl.yjiedai WHERE DATE >= '2012-01-01' AND class IN('03A') GROUP BY date ORDER BY DATE,class;
	...
	SELECT arg_hotel_group_id,arg_hotel_id,DATE,order_,itemno,MODE,'02G','团体','团体',NULL,
		SUM(last_charge),SUM(last_credit),SUM(charge),SUM(credit),SUM(apply),SUM(till_charge),SUM(till_credit),
		SUM(last_chargem),SUM(last_creditm),SUM(chargem),SUM(creditm),SUM(applym),SUM(till_chargem),SUM(till_creditm)
	FROM migrate_yl.yjiedai WHERE DATE >= '2012-01-01' AND class IN('02G') GROUP BY date ORDER BY DATE,class;
