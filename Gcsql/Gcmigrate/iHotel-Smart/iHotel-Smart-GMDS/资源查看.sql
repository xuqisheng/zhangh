...
SELECT a.* FROM  rsv_src AS a WHERE hotel_group_id = 1 AND hotel_id = 1 AND occ_flag = 'MF' AND dep_date = '2014-11-06'
AND NOT EXISTS (SELECT 1 FROM real_time_room_sta WHERE hotel_group_id = 1 AND hotel_id = 1 
AND rmno = a.rmno  AND is_dep= 'T')


SELECT id,accnt,rmno,master_id,arr_date,dep_date FROM rsv_src WHERE hotel_group_id = 1 AND hotel_id = 1 
AND occ_flag = 'MF' AND  dep_date = '2014-11-06'
