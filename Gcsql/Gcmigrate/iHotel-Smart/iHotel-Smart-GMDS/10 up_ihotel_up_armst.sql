...
BEGIN
	...
	DECLARE c_armst CURSOR FOR SELECT a.accnt,b.name,a.artag1,a.artag2,a.arr,a.dep,a.ref,b.phone,b.fax,b.mobile,
		a.charge,a.credit,a.charge-a.credit,a.accredit,b.sno,a.cby,a.changed,b.liason,b.zip,a.resby,a.restime 
		FROM migrate_db.master a,migrate_db.guest b 
		WHERE a.haccnt=b.no AND a.accnt LIKE 'AR%' ORDER BY a.accnt;
	...
	FETCH c_armst INTO var_accnt,var_name,var_artag1,var_artag2,var_arr,var_dep,var_ref,var_phone,var_fax,var_mobile,var_charge,var_credit,var_balance,var_accredit,var_sno,var_cby,var_changed,var_liason,var_zip,var_resby,var_restime; 
	WHILE done_cursor = 0 DO
		BEGIN			
				...
				VALUES(arg_hotel_group_id,arg_hotel_id,var_accnt,var_profile_type,var_profile_id,var_biz_date,'I',var_arr,var_dep,'','I','','###',
					...
					var_phone,var_fax,'',var_charge,var_credit,var_accredit,0,0,NULL,var_ref,'','',
					'',NULL,'',NULL,'',NULL,'',NULL,
				...
				INSERT INTO ar_master_guest(hotel_group_id,hotel_id,id,NAME,name2,name_combine,coding,LANGUAGE,nation,linkman,address,zipcode,create_user,create_datetime,modify_user,modify_datetime)
					VALUES(arg_hotel_group_id,arg_hotel_id,var_id,var_name,'',var_name,var_accnt,'C','CN',var_liason,'',var_zip,var_cby,var_changed,var_cby,var_changed);
	...
	CALL up_ihotel_up_armst_account(arg_hotel_group_id,arg_hotel_id);
	
	UPDATE ar_master a,up_map_code b SET a.pay_code = b.code_new WHERE a.hotel_group_id=arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND b.hotel_group_id=arg_hotel_group_id AND b.hotel_id = arg_hotel_id AND b.code IN ('pccode') AND b.code_old = a.pay_code; 
