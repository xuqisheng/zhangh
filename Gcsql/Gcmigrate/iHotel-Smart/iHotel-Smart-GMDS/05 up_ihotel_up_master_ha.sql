...
DROP PROCEDURE IF EXISTS `up_ihotel_up_master_ha`$$

CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_up_master_ha`(
...
BEGIN
	...
	DECLARE c_master CURSOR FOR 
		SELECT a.accnt,b.no,a.type,a.roomno,a.arr,a.dep,a.cusno,a.agent,a.source,TRIM(b.name),IF(b.nation IN('CN','TW'),TRIM(b.fname),TRIM(b.lname)),IF(b.nation IN('CN','TW'),TRIM(b.lname),TRIM(b.fname)),TRIM(b.name2),TRIM(b.name3),
			...
			IF(b.birth >= DATE_ADD(NOW(),INTERVAL -1 DAY),NULL,b.birth),b.vip,b.idcls,b.ident,b.nation,b.race,b.street,b.city,a.charge-a.credit,a.changed,a.phone,a.fax,a.email,
