...
DROP PROCEDURE IF EXISTS `up_ihotel_up_guest`$$

CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_up_guest`(
...
BEGIN
	...
	DECLARE var_address1		VARCHAR(40);
	DECLARE var_address2		VARCHAR(40);
	DECLARE var_CertNO		CHAR(20);
	DECLARE var_mobile	VARCHAR(15);
	DECLARE var_comments	VARCHAR(50);
	DECLARE var_message1	CHAR(255);
	DECLARE var_message2	CHAR(255);
	DECLARE var_message_ot	CHAR(255);
	DECLARE var_guest_id INT;
	DECLARE done_cursor INT DEFAULT 0;
	
	DECLARE c_cursor CURSOR FOR SELECT a.id,a.name,IF(a.sex = '','1','2') sex,SUBSTRING(id_no,7,8) birth,a.id_code,a.id_no,'CN',a.street,a.mobile,a.comments 
	FROM migrate_db.guest a;
	
	DECLARE CONTINUE HANDLER FOR SQLSTATE '02000' SET done_cursor = 1;
	DELETE FROM up_status WHERE hotel_id=arg_hotel_id AND up_step='GUEST';
	INSERT INTO up_status(hotel_id,up_step,time_begin,time_end,time_long,remark) VALUES(arg_hotel_id,'GUEST', NOW(),NULL,0,''); 
		
	OPEN c_cursor;
	FETCH c_cursor INTO var_accnt,var_name,var_sex,var_birth,var_idtype,var_idno,var_nation,var_address,var_mobile,var_comments;
	WHILE done_cursor <> 1 DO
		...
		IF NOT EXISTS(SELECT 1 FROM guest_link_base WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = 0 AND id = var_guest_id) THEN
			...
			VALUES(arg_hotel_group_id,0,var_guest_id,var_mobile,'','','','','','','','',
		...
		IF NOT EXISTS(SELECT 1 FROM guest_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND guest_id = var_guest_id) THEN
			...
			VALUES(arg_hotel_group_id,arg_hotel_id,var_guest_id,'I',var_accnt,'F','','','',
				...
				'000000000000000000000000000000','',var_comments,'ADMIN',NOW(),'ADMIN',NOW());
		END IF;
		-- 集团客史档案
		IF NOT EXISTS(SELECT 1 FROM guest_type WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND guest_id = var_guest_id) THEN
			INSERT INTO guest_type (hotel_group_id, hotel_id, guest_id, sta, manual_no, sys_cat, flag_cat, grade, latency, 
	...
	UPDATE up_status SET time_end=NOW() WHERE hotel_id=arg_hotel_id AND up_step='GUEST';
	UPDATE up_status SET time_long=TIMESTAMPDIFF(SECOND,time_begin,time_end) WHERE hotel_id=arg_hotel_id AND up_step='GUEST';
