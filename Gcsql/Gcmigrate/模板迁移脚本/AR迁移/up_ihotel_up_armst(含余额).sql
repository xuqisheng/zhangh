...
BEGIN
	...
	DECLARE c_armst CURSOR FOR SELECT accnt,name1,IFNULL(artype,'LC'),'',begin_,end_,IF(remark='NULL','',remark),'',LEFT(IF(IFNULL(phone,'')='NULL','',IFNULL(phone,'')),20),LEFT(IF(IFNULL(fax,'')='NULL','',IFNULL(fax,'')),20),
	IFNULL(balance,0),'I',IFNULL(IF(street='NULL','',street),''),'','',IFNULL(IF(email='NULL','',email),''),'','ADMIN_YQ',NOW() 
	...
	-- 提取余额录入入帐代码信息 
 --	SET var_ta_code = '8000', var_ta_code_des = '切换余额',var_ta_code_des_en = '切换余额',var_arg_code='30'; 
	-- 提取余额 消费和付款分开提取
