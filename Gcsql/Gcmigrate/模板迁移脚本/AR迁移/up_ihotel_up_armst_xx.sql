...
BEGIN
	...
	DECLARE var_remark		VARCHAR(255);
	DECLARE	var_linkman	  	VARCHAR(100);
 	
 	DECLARE c_armst CURSOR FOR SELECT a.arno,a.name,'I',a.arr,a.dep,a.balance,a.remark,a.link_man
	FROM migrate_db.armst a;
	...
	-- 提取余额录入入帐代码信息 
 	SET var_ta_code = '11000', var_arg_code='35'; 
	...
	-- 开始迁移，光标扫描 
 	...
 	OPEN c_armst ;
	SET done_cursor = 0 ;
	FETCH c_armst INTO var_accnt,var_name,var_status,var_begin,var_end,var_balance,var_remark,var_linkman; 
...
-- 				END IF; 
				...
				VALUES(var_hotel_group_id, var_hotel_id, var_accnt, var_profile_type, var_profile_id, var_biz_date, var_status, var_begin,var_end, '', 'I', '', '###', 
					'1', 'A', 'F', '1', IF(var_limit_type = '1','001000000000000000000000000000','000000000000000000000000000000'), '11000', var_limit_type, 0, '', '', '', '', 
					'', '', '', var_balance, 0, 0, 0, 0, NULL, var_remark, '', '', 
					'', NULL, '', NULL, '', NULL, '', NULL, 
				...
				VALUES(var_hotel_group_id, var_hotel_id,var_id,var_name,var_name,var_name,'','C','CN',var_linkman,'','ADMIN', NOW(), 'ADMIN', NOW());
...
-- 	UPDATE ar_master a,up_map_code b SET a.salesman = b.code_new WHERE b.hotel_group_id = var_hotel_group_id AND b.hotel_id = var_hotel_id AND b.code = 'salesman' AND b.code_old = a.salesman;
	...
	INSERT INTO ar_account (hotel_group_id,hotel_id,accnt,subaccnt,number,inumber,modu_code,biz_date,gen_date,ta_code,arrage_code,quantity,charge,pay,balance,charge0,pay0, 
		...
		SELECT var_hotel_group_id,var_hotel_id, b.id, c.id,1, 0, '02', var_biz_date, var_biz_date, var_ta_code,var_arg_code, 1, 0, 0, 0 ,b.charge,b.pay,
		0,0,0,0,0,0,'',0,a.name,a.name,'1','','','',0,0,
		var_ta_code_des,var_ta_code_des_en,'','','','',0,'A',1,NULL,NULL,NULL,NULL,NULL,
		NULL,'ADMIN',NOW(),'ADMIN',NOW()
		FROM ar_master b,ar_account_sub c,ar_master_guest a WHERE b.hotel_group_id = var_hotel_group_id AND b.hotel_id = var_hotel_id
		AND c.hotel_group_id = var_hotel_group_id AND c.hotel_id = var_hotel_id AND c.type ='SUBACCNT' AND b.id = c.accnt
		AND b.id = a.id AND a.hotel_group_id = var_hotel_group_id AND a.hotel_id =var_hotel_id; 
	...
	SELECT var_hotel_group_id,var_hotel_id,b.id, c.id,1,1, '02',var_biz_date,var_biz_date,var_ta_code,var_arg_code, '', 1, b.charge,b.charge,
	...
	FROM  ar_master b,ar_account_sub c WHERE b.hotel_group_id = var_hotel_group_id AND b.hotel_id = var_hotel_id 
	AND c.hotel_group_id = var_hotel_group_id AND c.hotel_id = var_hotel_id AND c.type = 'SUBACCNT' AND b.id = c.accnt; 
	UPDATE ar_master a,(SELECT accnt,MAX(ar_number) ar_number FROM ar_detail WHERE hotel_group_id = var_hotel_group_id AND hotel_id = var_hotel_id GROUP BY accnt) b
	SET a.last_num_link = b.ar_number + 1  WHERE a.hotel_group_id = var_hotel_group_id AND a.hotel_id = var_hotel_id AND a.id = b.accnt;
	UPDATE ar_master a,(SELECT accnt,MAX(ar_inumber) ar_inumber FROM ar_detail WHERE hotel_group_id = var_hotel_group_id AND hotel_id = var_hotel_id GROUP BY accnt) b
	SET a.last_num = b.ar_inumber + 1 WHERE a.hotel_group_id = var_hotel_group_id AND a.hotel_id = var_hotel_id AND a.id = b.accnt;
