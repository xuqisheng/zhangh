...
DROP PROCEDURE IF EXISTS `up_ihotel_up_armst_xx`$$

CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_up_armst_xx`(
...
BEGIN
	...
	DECLARE var_ta_code_des_en 	VARCHAR(60);
 	...
 	DECLARE c_armst CURSOR FOR SELECT a.accnt,a.name,a.ar_type,a.begin,a.end,a.linkman,a.mobile     
	FROM migrate_xx.armst a;
	...
	-- 开始迁移，光标扫描 
 	...
 	OPEN c_armst ;
	SET done_cursor = 0 ;
	FETCH c_armst INTO var_accnt,var_name,var_artype,var_begin,var_end,var_linkman,var_mobile; 
...
-- 				END IF; 
				...
				VALUES(var_hotel_group_id, var_hotel_id,var_id,var_name,var_name,var_name,'','C','CN',IFNULL(var_linkman,''),'','ADMIN', NOW(), 'ADMIN', NOW());
