...
DROP PROCEDURE IF EXISTS `ihotel_up_master_si_v5`$$

CREATE DEFINER=`root`@`%` PROCEDURE `ihotel_up_master_si_v5`(
...
BEGIN
	...
	/*
	 消费 rmb_db 
	 付款  depr_cr  +  addrmb 
	 客欠  rmb_db  -  depr_cr  -  addrmb 
	 class = E独占,F共享,L长包,Z 自用 
	 exp_s - 结算方式
	 paycode - 支付方式 
	 resmode 预订方式
	 credman 销售员
	...
	*/	
	DECLARE c_master CURSOR FOR 
		SELECT a.accnt,b.guestid,a.type,a.roomno,a.rmnum,a.arr,a.dep,a.gstno,a.children,a.cusno,a.tranlog,a.src,a.mkt,
			...
			b.race,b.address,b.birthplace,a.rmb_db-a.depr_cr-a.addrmb,a.sta,c.content,d.content,e.content,f.content,a.credman,a.pcrec,a.applname,a.applicant,a.resby,a.reserved,a.cby,a.resno,a.changed
		FROM migrate_xc.master a,migrate_xc.guest b 
		LEFT JOIN migrate_xc.message c ON b.accnt = c.accnt AND c.type = '61'
		LEFT JOIN migrate_xc.message d ON b.accnt = c.accnt AND c.type = '64'
		LEFT JOIN migrate_xc.message e ON b.accnt = c.accnt AND c.type = '63'
		LEFT JOIN migrate_xc.message f ON b.accnt = c.accnt AND c.type = '73'		
		WHERE a.accnt=b.accnt AND SUBSTRING(a.accnt,2,2) <> '95' AND a.sta IN ('S','I','O') ORDER BY a.accnt;
	...
	DELETE FROM up_map_accnt WHERE hotel_group_id=arg_hotel_group_id AND hotel_id=arg_hotel_id AND accnt_type='master_si' AND (accnt_class='S' OR accnt_class='I');
	...
	FETCH c_master INTO var_accnt,var_guestid,var_rmtype,var_rmno,var_rmnum,var_arr,var_dep,var_gstno,var_children,var_cusno,var_tranlog,var_src,var_mkt,
					...
					var_race,var_address,var_birthplace,var_balance,var_sta,var_content,var_content2,var_content3,var_content4,var_credman,var_pcrec,var_applname,var_applicant,var_resby,var_reserved,var_cby,var_resno,var_changed; 
		...
		BEGIN
			...
			-- 插入登记单.帐户对照关系保存在 sc_flag 
				...
				VALUES(arg_hotel_group_id,arg_hotel_id,0,IF(var_rmno<>'','F','T'),var_applname,var_applicant,0,'F',0,0,0,var_biz_date,
					var_sta,var_rmtype,var_rmno,var_rmnum,var_arr,var_dep,var_gstno,var_children,'I',NULL,0,0,0,var_rtreason,var_qtrate-var_setrate,0,
					'I',var_sta,'F','F','',var_cid,var_aid,var_sid,'','',var_credman,'','',
					var_mkt,var_src,var_rsv_type,var_channel,var_tranlog,'','',var_srqs,'',
					'F','F','T','F','F','0','F','0',
					CONCAT(var_accnt,'-',var_guestid),'000000000000000000000000000000',0,0,0,0,IFNULL(var_paycode,'10002'),0,'',CONCAT(var_applname,var_phoneetc),'',
					0,0,0,1,0,0,0,0,var_resno,'',
					var_wherefrom,var_whereto,'',CONCAT(IFNULL(var_content,''),IFNULL(var_content2,'')),CONCAT(IFNULL(var_content3,''),IFNULL(var_content4,'')),var_resby,var_reserved,var_cby,var_changed);
			ELSE
				INSERT INTO master_base (hotel_group_id,hotel_id,rsv_id,is_resrv,rsv_man,rsv_company,rsv_src_id,rsv_class,master_id,grp_accnt,block_id,biz_date,
					sta,rmtype,rmno,rmnum,arr,dep,adult,children,res_sta,res_dep,rack_rate,nego_rate,real_rate,dsc_reason,dsc_amount,dsc_percent,
					exp_sta,tm_sta,rmpost_sta,is_rmposted,tag0,company_id,agent_id,source_id,member_type,member_no,salesman,arno,building,
					...
					where_from,where_to,purpose,remark,co_msg,create_user,create_datetime,modify_user,modify_datetime)
				VALUES(arg_hotel_group_id,arg_hotel_id,0,IF(var_rmno<>'','F','T'),var_applname,var_applicant,0,'F',0,0,0,var_biz_date,
					var_sta,var_rmtype,var_rmno,var_rmnum,var_arr,var_dep,var_gstno,var_children,'I',NULL,var_qtrate,var_setrate,var_setrate,var_rtreason,var_qtrate-var_setrate,0,
					'I',var_sta,'F','F','',var_cid,var_aid,var_sid,'','',var_credman,'','',
					var_mkt,var_src,var_rsv_type,var_channel,var_tranlog,'','',var_srqs,'',
					'F','F','T','F','F','0','F','0',
					CONCAT(var_accnt,'-',var_guestid),'000000000000000000000000000000',0,0,0,0,IFNULL(var_paycode,'10002'),0,'',CONCAT(var_applname,var_phoneetc),'',
			...
			VALUES(arg_hotel_group_id,arg_hotel_id,var_id,var_resby,var_reserved,var_cby,var_changed,
				IF(var_sta = 'I','',var_cby),IF(var_sta = 'I',NULL,var_changed),IF(var_sta = 'I','',var_cby),IF(var_sta = 'I',NULL,var_changed),var_cby,var_changed);
	...
	INSERT INTO up_map_accnt(hotel_group_id,hotel_id,accnt_type,accnt_class,accnt_old,accnt_new) 
		SELECT arg_hotel_group_id,arg_hotel_id,'master_si',CONCAT(rsv_class,sta),sc_flag,id FROM master_base
			WHERE hotel_group_id=arg_hotel_group_id AND hotel_id=arg_hotel_id AND sc_flag<>'' AND sc_flag IS NOT NULL AND rsv_class <> 'H' AND sta IN ('S','I','O');
