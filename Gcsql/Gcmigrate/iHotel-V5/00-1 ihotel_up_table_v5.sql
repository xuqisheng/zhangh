...
CREATE TABLE up_map_accnt (
  ...
  PRIMARY KEY (id),
  UNIQUE KEY Index_1 (hotel_group_id,hotel_id,accnt_type,accnt_class,accnt_old),
  UNIQUE KEY Index_2 (hotel_group_id,hotel_id,accnt_type,accnt_class,accnt_new)
...
CREATE TABLE up_map_code (
  hotel_group_id	BIGINT(16)	NOT NULL,
  hotel_id 			BIGINT(16) 	NOT NULL, 
  cat 				VARCHAR(12) NOT NULL COMMENT '比如：性别=sex 预订类型=restype',
  ...
  UNIQUE KEY Index_1(hotel_group_id,hotel_id,cat,code_old)
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=utf8 COMMENT='所有代码映射表';
