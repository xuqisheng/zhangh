-- 房类
SELECT * FROM master_base WHERE hotel_group_id = 2 AND  hotel_id=9 AND rmtype<>''
AND rmtype NOT IN (SELECT CODE FROM room_type WHERE hotel_group_id = 2 AND  hotel_id=9 )

SELECT * FROM rsv_src WHERE hotel_group_id = 2 AND  hotel_id=9 AND rmtype<>''
AND rmtype NOT IN (SELECT CODE FROM room_type WHERE hotel_group_id = 2 AND  hotel_id=9 )

-- 市场码
SELECT * FROM master_base WHERE hotel_group_id = 2 AND  hotel_id=9
AND market NOT IN (SELECT CODE FROM code_base WHERE hotel_group_id = 2 AND  hotel_id=9 AND parent_code='market_code' )
GROUP BY market

-- 来源码
SELECT  * FROM master_base WHERE hotel_group_id = 2 AND  hotel_id=9  
AND src NOT IN (SELECT CODE FROM code_base WHERE hotel_group_id = 2 AND  hotel_id=9 AND parent_code='src_code' AND is_halt='F')
GROUP BY src

-- 渠道码
SELECT * FROM master_base WHERE hotel_group_id = 2 AND  hotel_id=9
AND channel NOT IN (SELECT CODE FROM code_base WHERE hotel_group_id = 2 AND  hotel_id=9 AND parent_code='channel' AND is_halt='F') GROUP BY channel

-- 预订类型
SELECT * FROM master_base WHERE hotel_group_id = 2 AND  hotel_id=9  AND rsv_class<>'H'
AND rsv_type NOT IN (SELECT CODE FROM code_rsv_type WHERE hotel_group_id = 2 AND  hotel_id=9 AND is_halt='F') 
GROUP BY rsv_type

-- 房价码
SELECT * FROM master_base WHERE hotel_group_id = 2 AND  hotel_id=9  AND ratecode<>''  AND rsv_class<>'H'
AND ratecode NOT IN (SELECT CODE FROM code_ratecode WHERE hotel_group_id = 2 AND  hotel_id=9 ) 
GROUP BY ratecode
