...
DROP PROCEDURE IF EXISTS `ihotel_up_master_ha_x5`$$

CREATE DEFINER=`root`@`%` PROCEDURE `ihotel_up_master_ha_x5`(
...
BEGIN
	...
	DECLARE c_master CURSOR FOR 
		SELECT a.accnt,b.no,a.type,a.roomno,a.arr,a.dep,a.cusno,a.agent,a.source,TRIM(b.name),IF(b.nation IN('CN','TW'),TRIM(b.fname),TRIM(b.lname)),IF(b.nation IN('CN','TW'),TRIM(b.lname),TRIM(b.fname)),TRIM(b.name2),TRIM(b.name3),TRIM(b.name4),
			...
			b.birth,b.vip,b.idcls,b.ident,b.nation,b.race,b.street,b.city,a.charge-a.credit,a.changed,a.phone,a.fax,a.email,
			a.sta,a.class,a.pcrec,TRIM(a.comsg),IFNULL(a.cby,'ADMIN'),a.restype,a.channel,a.resno,a.crsno,a.gstno,a.children,a.applname,a.applicant
		FROM migrate_db.master a,migrate_db.guest b WHERE a.haccnt=b.no AND a.class='C' AND a.sta IN ('I','S','O') ORDER BY a.accnt;
	...
	DELETE FROM up_map_accnt WHERE hotel_group_id=arg_hotel_group_id AND hotel_id=arg_hotel_id AND accnt_type='consume';
	
	OPEN c_master ;
	SET done_cursor = 0 ;	
	FETCH c_master INTO var_accnt,var_guestid,var_rmtype,var_rmno,var_arr,var_dep,var_cusno,var_agent,var_source,var_name,var_lname,var_fname,var_name2,var_name3,var_name4,
					...
					var_birth,var_vip,var_idcls,var_ident,var_nation,var_race,var_address,var_birthplace,var_balance,var_changed,var_phone,var_fax,var_email,
	...
	WHILE done_cursor = 0 DO
		BEGIN
			...
			IF var_profile_id=0 AND var_ident<>'' THEN 
				...
				CALL ihotel_up_get_guest_id_yw(arg_hotel_group_id,arg_hotel_id,var_haccnt,var_name,var_idcls,var_ident,
			...
			ELSE
				INSERT INTO master_guest (hotel_group_id,hotel_id,id,profile_type,profile_id,times_in,name,last_name,first_name,name2,name3,name_combine,
					...
					vip,phone,mobile,fax,email,id_code,id_no,id_end,visa_type,visa_no,visa_begin,visa_end,visa_grant,enter_port,enter_date,enter_date_end,
					photo_pic,photo_sign,remark,create_user,create_datetime,modify_user,modify_datetime)
				VALUES(arg_hotel_group_id,arg_hotel_id,var_id,'GUEST',0,0,var_name,var_lname,var_fname,var_name2,var_name3,var_name4,
	...
	UPDATE master_base SET dep = DATE_ADD(NOW(),INTERVAL 3 YEAR) WHERE hotel_group_id = arg_hotel_group_id AND hotel_id = arg_hotel_id AND rsv_class = 'H' AND dep <= NOW();
