...
DROP PROCEDURE IF EXISTS `up_ihotel_up_master_si`$$

CREATE DEFINER=`root`@`%` PROCEDURE `up_ihotel_up_master_si`(
...
BEGIN
	...
	DECLARE c_master CURSOR FOR 
		SELECT a.accnt,b.no,a.type,a.roomno,a.arr,a.dep,a.cusno,a.agent,a.source,a.cardcode,a.cardno,a.saleid,TRIM(b.name),IF(b.nation IN('CN','TW'),TRIM(b.fname),TRIM(b.lname)),IF(b.nation IN('CN','TW'),TRIM(b.lname),TRIM(b.fname)),TRIM(b.name2),TRIM(b.name3),
			a.ratecode,a.src,a.market,a.qtrate,a.rmrate,a.setrate,a.rtreason,a.addbed,a.addbed_rate,a.crib,a.crib_rate,a.paycode,a.srqs,TRIM(a.ref),b.no,b.sex,a.amenities,a.up_type,a.up_reason,b.visaid,IF(b.visaend > DATE_ADD(NOW(),INTERVAL -1 DAY),NULL,b.visaend),b.rjplace,IF(b.rjdate >= DATE_ADD(NOW(),INTERVAL -1 DAY),NULL,b.rjdate),
			IF(b.birth >= DATE_ADD(NOW(),INTERVAL -1 DAY),NULL,birth),b.vip,b.idcls,b.ident,b.nation,b.race,TRIM(b.street),b.city,a.extra,a.charge-a.credit,b.i_times,a.changed,a.phone,b.mobile,a.fax,a.email,b.feature,
		...
		FROM migrate_db.master a,migrate_db.guest b WHERE a.haccnt=b.no AND a.class IN ('F','G','M') 
			AND a.sta IN ('I','O','S') AND a.exp_s2 = arg_hotel_id ORDER BY a.accnt;
	...
	FETCH c_master INTO var_accnt,var_guestid,var_rmtype,var_rmno,var_arr,var_dep,var_cusno,var_agent,var_source,var_cardcode,var_cardno,var_saleid,var_name,var_lname,var_fname,var_name2,var_name3,
					var_ratecode,var_src,var_mkt,var_qtrate,var_rmrate,var_setrate,var_rtreason,var_addbed,var_addbed_rate,var_crib,var_crib_rate,var_paycode,var_srqs,var_ref,var_haccnt,var_sex,var_amenities,var_uptype,var_upreason,var_visaid,var_visaend,var_rjplace,var_rjdate,
					var_birth,var_vip,var_idcls,var_ident,var_nation,var_race,var_address,var_birthplace,var_extra,var_balance,var_itimes,var_changed,var_phone,var_mobile,var_fax,var_email,var_feature,
		...
		BEGIN
			...
			IF var_accnt = var_last_accnt THEN 				
				...
				VALUES(arg_hotel_group_id,arg_hotel_id,0,'F',var_applname,var_applicant,0,var_class,0,0,0,var_biz_date,
					...
					'F','F','T','F','F',SUBSTRING(var_extra,4,1),IF(SUBSTRING(var_extra,5,1) = 1,'T','F'),'0',
			...
			IF var_profile_id > 0 THEN 
				...
				SELECT arg_hotel_group_id,arg_hotel_id,var_id,'GUEST',a.id,var_itimes,a.name,a.last_name,a.first_name,a.name2,a.name3,a.name_combine,
					...
					d.vip,var_phone,var_mobile,var_fax,var_email,a.id_code,a.id_no,a.id_end,var_visaid,'',NULL,var_visaend,'',var_rjplace,var_rjdate,NULL,
			...
			ELSE
				...
				VALUES(arg_hotel_group_id,arg_hotel_id,var_id,'GUEST',0,var_itimes,var_name,var_lname,var_fname,var_name2,var_name3,CONCAT(var_name,var_name2),
					...
					var_vip,var_phone,var_mobile,var_fax,var_email,var_idcls,var_ident,NULL,var_visaid,'',NULL,var_visaend,'',var_rjplace,var_rjdate,NULL,
					0,0,'','',var_feature,var_cby,var_changed,var_cby,var_changed);
			END IF; 			
	...
	INSERT INTO up_map_accnt(hotel_group_id,hotel_id,accnt_type,accnt_class,accnt_old,accnt_new) 
		SELECT arg_hotel_group_id,arg_hotel_id,'master_si',CONCAT(rsv_class,sta),sc_flag,id FROM master_base
			WHERE hotel_group_id=arg_hotel_group_id AND hotel_id=arg_hotel_id AND sc_flag<>'' 
				AND sc_flag IS NOT NULL AND sta IN ('I','S','O') AND rsv_class IN ('F','G');
