if exists(select * from sysobjects where name = "p_gds_reserve_rmsta_list" and type = 'P')
	drop proc p_gds_reserve_rmsta_list;
create  proc p_gds_reserve_rmsta_list
	@hall		 char(1),
	@rm_no	 char(5),
	@brm_no	 char(5),
	@type		 char(5),
...
declare	c_rmsta_list cursor for
	...
	from 	rmsta
	where roomno like rtrim(@rm_no)+'%' 
			and oroomno >= @brmno
			and (hall=@hall or rtrim(@hall) is null) 
			and type like rtrim(@type) +'%' 
			and ocsta like rtrim(@ocsta)+'%' 
			and sta like rtrim(@sta)+'%' 
			and feature like @feature 
			and (@bed is null or bedno = @bed or @bed=0)
