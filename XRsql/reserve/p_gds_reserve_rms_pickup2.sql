IF OBJECT_ID('p_gds_reserve_rms_pickup2') IS NOT NULL
    DROP PROCEDURE p_gds_reserve_rms_pickup2
;
create proc p_gds_reserve_rms_pickup2
	@repdate		datetime 
as
-- 针对历史数据的处理  - 收益经理需要 
create table #pickup (
...
select @begin=convert(datetime, substring(convert(char(10),@repdate,111), 1, 8)+'01')
...
begin
	...
	if @begin<@repdate 
	begin
--		select @occ = amount from yaudit_impdata where date=@begin and class='sold' 
--		select @revenue = amount from yaudit_impdata where date=@begin and class='income' 
--		select @rebate = day07 from yjierep where date=@begin and class='010' 
		select @occ = 0
		select @revenue = 0
		select @rebate = 0
	...
	begin
		exec p_gds_reserve_rsv_index_his @repdate, @begin, @occ output, @revenue output, @ten_rn output, @ten_rev output
