...
select @types=rtrim(@types)
if substring(@types,datalength(@types),1)=',' 
   select @types=substring(@types,1,datalength(@types) -1)
...
insert #grid (date, type)
	select @larr, type from typim where @types='%' or charindex(','+rtrim(type)+',', ','+rtrim(@types)+',')>0
