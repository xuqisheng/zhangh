IF OBJECT_ID('p_gds_reserve_grid_block_pre') IS NOT NULL
    DROP PROCEDURE p_gds_reserve_grid_block_pre
;
create proc p_gds_reserve_grid_block_pre
	@accnt		char(10),
...
begin
	insert #grid (date, type) select @larr, type from typim
		where tag='K' and (@types='%' or charindex(','+type+',', ','+@types+',')>0 ) 
