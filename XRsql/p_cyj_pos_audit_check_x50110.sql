if exists(select 1 from sysobjects where name ='p_cyj_pos_audit_check_x50110' and type ='P' )
drop  proc p_cyj_pos_audit_check_x50110;
create proc p_cyj_pos_audit_check_x50110
	@date		datetime
as
----------------------------------------------------------------------------------------------*/
--
-- 餐饮数据检查，会导致底表不平的相关因素
-- 适用范围 x50109, x50110   cyj 2010.03.31
----------------------------------------------------------------------------------------------*/
...
-- itemdef没有定义报表项目
select @count1 = count(1) from pos_itemdef a,pos_pccode b, pccode c where a.pccode=b.pccode  and b.pccode=c.pccode
	and a.code not in( select d.code from  pos_namedef d where d.deptno=c.deptno and a.code=d.code)
if @count1 > 0 
	insert #list
	select '110','itemdef没有定义的报表代码'+'pos_itemdef','','部门号:'+c.deptno+' 餐厅:'+a.pccode+'代码:'+a.code from pos_itemdef a,pos_pccode b, pccode c where a.pccode=b.pccode  and b.pccode=c.pccode
	and a.code not in( select d.code from  pos_namedef d where c.deptno=d.deptno and a.code=d.code)
