...
if @code_new='' or @code_new = null
	...
	select @code_new=substring(@code_new,1,7)+substring('000'+rtrim(convert(char(3),convert(integer,substring(@code_new,8,3))+1)),datalength('000'+rtrim(convert(char(3),convert(integer,substring(@code_new,8,3))+1))) -2,3)
	end
