...
if object_id("p_gds_reserve_mkt_cat_forecast") is not null
	drop proc p_gds_reserve_mkt_cat_forecast;
create proc p_gds_reserve_mkt_cat_forecast
...
if exists(select 1 from sysoption where catalog = 'hotel' and item = 'allotment' and charindex(rtrim(value),'TYty') > 0 )
	exec p_zk_room_plan_check @pc_id,@s_time,@e_time,'%','D'

delete forecast where pc_id = @pc_id
...
---------------------------------------------------------------------------------------------------------
--	insert forecast
--		select @id, @s_time, '', right('0000'+rtrim(convert(char(10),a.sequence)),4), a.code, '',0
--			from mktcode a
...
---------------------------------------------------------------------------------------------------------
	...
	insert #mkt select b.market, count(distinct a.roomno), 0 from rsvsrc_detail a, master b 
			where b.sta in ('I','R') and a.accnt=b.accnt and a.roomno<>'' and datediff(dd,a.arr,@s_time)>=0 and datediff(dd,a.dep,@s_time)<0 and a.date_=@s_time 
				group by b.market
	...
	update #mkt set quan=quan + isnull((select count(distinct a.saccnt) from rsvsrc_detail a, master b 
															where b.sta in ('I','R') and a.accnt=b.accnt and a.roomno='' and b.market=#mkt.code and a.arr <> a.dep
																and datediff(dd,a.arr,@s_time)>=0 and datediff(dd,a.dep,@s_time)<0 and date_=@s_time and a.quantity=1),0)
									  ...
									  + isnull((select sum(a.quantity) from rsvsrc a, sc_master b 
															where a.accnt=b.accnt and a.roomno='' and b.market=#mkt.code and a.begin_ <> a.end_
																and a.begin_<=@s_time and a.end_>@s_time),0)
//	update #mkt set quan=quan + isnull((select sum(a.quantity) from rsvsaccnt a, master b where a.begin_<>a.end_ 
//									and a.begin_ <=@s_time and a.end_ > @s_time and a.type not in (select type from typim where tag='P') and a.accnt=b.accnt
//											and b.market=#mkt.code ),0)
//	update #mkt set quan=quan + isnull((select sum(a.quantity) from rsvsaccnt a, sc_master b where a.begin_<>a.end_ 
//									and a.begin_ <=@s_time and a.end_ > @s_time and a.type not in (select type from typim where tag='P') and a.accnt=b.accnt
//											and b.market=#mkt.code ),0)
...
update forecast set des = rtrim((select max(grp) from mktcode a,forecast b where rtrim(b.type) = rtrim(a.code) and b.sort = forecast.sort ))
 where sort not like '#%' and sort not like 'Z%' and type = '' and pc_id = @pc_id
update forecast set des = des +':'+ b.descript from basecode b where b.cat = 'market_cat' and b.code = forecast.des and pc_id = @pc_id
...
if @langid=0
	insert forecast
		select @pc_id,a.id, a.date, '', 'ZZ80', 'ZZZ', '   开房率3(%)', 
	...
	insert forecast
		select @pc_id,a.id, a.date, '','ZZ80', 'ZZZ', '   Occupancy3(%)', 
				round(a.quan*100.0/(select b.quan from forecast b where a.date=b.date and b.sort='#000' and pc_id = @pc_id),2)
			from forecast a where a.sort='#890' and pc_id = @pc_id
...
-- Occupancy1
select @value = b.quan from forecast b where b.sort='#000' and b.date='2020.01.01'  
if @value<> 0   
	update forecast set quan = round((select b.quan from forecast b where b.sort='#890' and b.date='2020.01.01' and pc_id = @pc_id)*100/@value,2)
		where datedes='TOTAL' and sort='ZZ80' and pc_id = @pc_id
-- AVR
select @value = b.quan from forecast b where b.sort='#890' and b.date='2020.01.01'
if @value<> 0
begin
...
delete from forecast where type in (select code from mktcode) and pc_id = @pc_id
delete from forecast where (des like '%日%' or des like '%+%' or des like '%3%' or des like '%在店客人%') and pc_id = @pc_id
