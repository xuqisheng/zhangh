if exists(select 1 from sysobjects where name = "p_gds_reserve_rep_o2")
   drop proc p_gds_reserve_rep_o2
;
create  proc p_gds_reserve_rep_o2
	@date1		datetime,
	@len			int = 6
as
...
--			from rsvtype where begin_ <= @pointer and end_ > @pointer
	...
	-- sold
	update #rep set sold = isnull((select sum(a.quantity) from rsvsaccnt a, master b, mktcode c
