IF OBJECT_ID('p_cq_sale_income_more') IS NOT NULL
    DROP PROCEDURE p_cq_sale_income_more
;
...
insert #goutput (saleid,accnt,sta,resno,haccnt,cusno)
	...
	from pos_hmenu a,saleid b,guest c where (a.haccnt = c.no or a.cusno = c.no ) and c.saleid = b.code and b.code like @saleid
...
select haccnt,name,i_guests, i_days,rm,fb,en,mt,ot,tl from #goutput
...
_com_p_协议单位业绩报表;
(exec p_cq_sale_income_more  '#char03!请输入销售员代码! #','#date1!请输入起始日期!#Bdate1##','#date2!请输入终止日期!#Bdate1##'resultset=char12, char60, mone10_1, mone10_2, mone10_3, mone10_5, mone10_6, mone10_7, mone10_8,mone10_9);
...
texttext=t_title1:协 议 单 位 业 绩 报 告 (按结帐日期):header:2::char12:mone10_9::border="0" alignment="2" font.height="-12" font.italic="1"!
