if object_id('p_hbb_forecast_report') is not null
	drop proc p_hbb_forecast_report;

create proc p_hbb_forecast_report
	@pc_id			char(4),
	@begin			datetime,
	@end				datetime,
	@langid			integer = 0
...
delete fct_order where pc_id = @pc_id
-- 使用代码次序表 fct_order
select @order_ = 1
declare c_code cursor for select distinct code from forecast_rep where pc_id = @pc_id order by code
open c_code
fetch c_code into @code
while @@sqlstatus = 0
	begin
	if not exists(select 1 from fct_order where pc_id = @pc_id and code = @code and order_ = @order_)	
		insert fct_order(pc_id,code,order_) values (@pc_id, @code, @order_)
	
	select @order_ = @order_ + 1	
	fetch c_code into @code
	end
close c_code
...
declare c_mkt cursor for select date,code,gstno,quantity,rate 
	from forecast_rep where pc_id = @pc_id order by date,code

open c_mkt
fetch c_mkt into @date,@code,@pquan,@rquan,@rincome
while @@sqlstatus = 0
	begin
--	select @vpos = charindex(@code, @codes)
--	select @vpos = (@vpos + 3) / 4
	select @vpos = order_ from fct_order where pc_id = @pc_id and code = @code
...
-- 计算合计平均房价
declare c_tmkt cursor for select code,sum(quantity),sum(rate) from forecast_rep where pc_id = @pc_id
...
--	select @vpos = (@vpos + 3) / 4
	...
	select @vpos = 0
	fetch c_tmkt into @code,@pquan,@rquan,@rincome
	end
close c_tmkt
deallocate cursor c_tmkt

-- 计算横向合计平均房价
select distinct date,rmrev = mkttt,rms = mkttt,adr = mkttt
	into #grep from mktrep where pc_id = @pc_id
update #grep set rmrev = 0,rms = 0,adr = 0

update #grep set rmrev = 
	isnull((select sum(a.mkttt) from mktrep a where #grep.date = a.date and a.pc_id = @pc_id and a.item = 'rmrev'),0)
update #grep set rms = 
	isnull((select sum(a.mkttt) from mktrep a where #grep.date = a.date and a.pc_id = @pc_id and a.item = 'rms'),0)
update #grep set adr = round(rmrev / rms,2) where rms != 0 
update mktrep set mkttt = a.adr from #grep a where mktrep.date = a.date and mktrep.pc_id = @pc_id and item = 'adr'
