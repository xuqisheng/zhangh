...
drop proc p_wz_house_map5_bu;
create proc p_wz_house_map5_bu
		@modu_id 		char(2),
		@pc_id			char(4)
as
declare
...
insert #tmp select a.flr from rmsta a ,hsmap_term_end b
	where b.modu_id = @modu_id and b.pc_id = @pc_id and a.roomno = b.roomno


insert #hs_flr select flr , count(1) from #tmp group by flr
select @column = b.colnum from sysoption a, hsmap_project b where  a.value = b.project and a.catalog='house' and a.item = 'project'
	...
	begin
	 if (@num % @column) <> 0
		begin
			...
			while @yu > 0
			 begin
				select @rmno =rtrim(@floor) + substring(convert(char(8),inttohex(160 + @yu)),7,2)
