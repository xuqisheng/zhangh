IF OBJECT_ID('p_hry_reserve_print_rmbmp') IS NOT NULL
    DROP PROCEDURE p_hry_reserve_print_rmbmp
;
create  proc  p_hry_reserve_print_rmbmp
   @pc_id   char(4),
   @modu_id char(2),
   @hall    varchar(20),
   @auto    char(1)
...
while @@sqlstatus = 0
   ...
   select @nocnt  = nocnt from nopart_count where pc_id = @pc_id and modu_id = @modu_id and no = @nopart
	...
	if @ocsta = 'O' and charindex(@sta,'OS') = 0
		...
		select @eccosta = 'DI'
	else if charindex(@sta,'OS')>0
		select @eccosta = 'OO'

   if @eccosta = 'OCC'
      if exists(select a.accnt from master a, mktcode b where a.sta='I' and a.market=b.code and b.flag='HSE' and a.roomno=@roomno)
         select @eccosta = 'HU'
