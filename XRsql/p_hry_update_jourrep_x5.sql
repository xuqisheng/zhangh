if exists(select 1 from sysobjects where name = 'p_hry_update_jourrep_x5' and type = 'P')
	drop proc p_hry_update_jourrep_x5;
create proc p_hry_update_jourrep_x5
	@bdate_b 	datetime,
	@bdate_e		datetime
as 
...
//		select @bdate
		...
		if @isfstday = 'T'
			update yjourrep set month = day,month_rebate = day_rebate where date = @bdate// and class <> '080000'
		else
			update yjourrep set 
            month = day + (select a.month from yjourrep a 
  				             where yjourrep.class = a.class and a.date = dateadd(day,-1,@bdate)),
            month_rebate = day_rebate + (select a.month_rebate from yjourrep a 
				             where yjourrep.class = a.class and a.date = dateadd(day,-1,@bdate))                 
			where yjourrep.date = @bdate

		if @isyfstday = 'T'
			update yjourrep set year = day,year_rebate = day_rebate where date = @bdate// and class <> '080000'
		else
			update yjourrep set 
            year = day + (select a.year from yjourrep a 
				            where yjourrep.class = a.class and a.date = dateadd(day,-1,@bdate)),
            year_rebate = day_rebate + (select a.year_rebate from yjourrep a 
				            where yjourrep.class = a.class and a.date = dateadd(day,-1,@bdate))
