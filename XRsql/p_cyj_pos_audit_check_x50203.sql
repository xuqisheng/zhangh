	drop  proc p_cyj_pos_audit_check_x50203;
create proc p_cyj_pos_audit_check_x50203
	@date		datetime
as
----------------------------------------------------------------------------------------------*/
--
-- 餐饮数据检查，会导致底表不平的相关因素
-- 适用范围 x50203 后的X系列, 包括 x50203, x50204, c7    cyj 2010.03.31
----------------------------------------------------------------------------------------------*/
...
-------- add by zmj or( a.tail>'06' and a.code<'100') 
--餐厅没有定义底表项目
select @count1 = count(1) from pos_itemdef a,pos_pccode b where a.pccode=b.pccode and (a.jierep not in(select class from jierep) or( a.tail>'06' and a.code<'100'))
if @count1 > 0 
	insert #list
	select distinct '130','餐厅没有定义底表项目'+'itemdef.jierep' + 'code' + a.code,'',' 餐厅:'+b.pccode+'底表行:'+a.jierep+ 'tial' + a.tail
	from pos_itemdef a,pos_pccode b where a.pccode=b.pccode and (a.jierep not in(select class from jierep) or( a.tail>'06' and a.code<'100'))
...
if @date = @bdate 
	begin
	insert into #menu select * from pos_menu where sta ='3'
	...
	begin
	insert into #menu select * from pos_hmenu where sta ='3' and bdate=@date
...
-----插入没有pay的menu记录，没有pay，对比要去关联#pay表，没有记录，就不关联了
insert #pay
select menu,9,9,'1111','','','' ,0,'0','','','FOX',bdate,shift,bdate,'',menu,'',0,'','',0 
from #menu where menu not in (select menu from #pay )
	end
...
insert #list 
select '810','dish Z 服务费不等于明细服务费合计',a.menu,convert(varchar,a.srv)+','+convert(varchar,(select b.amount from #dish b where a.menu=b.menu and b.code ='Z'))+','
	+convert(varchar,(select sum(c.srv) from #dish c where a.menu=c.menu and c.code <>'Y' and c.code <>'Z'))
	from #menu a where 
