...
insert #gout select '1000',pccode+descript+'：未对应底表行',0 from pccode where jierep not in (select class from jierep) and pccode <'9' 
...
insert #gout 
	select '1007','bos收入有3位小数'+code+descript,fee_ttl from bosjie where empno='{{{' and shift='9' and fee_ttl<>round(fee_ttl,2) and code <>'999'
...
insert #gout 
select 'pos1008','餐饮转应收与前台账务数据不符',(select sum(creditd) from deptdai where shift='9' and empno ='{{{' and paycode in ('C88','C86')) -
(select sum(charge) from gltemp where modu_id ='04')

insert #gout
select  'pos1010',a.menu+'餐单零头未生成',sum(a.amount)+sum(a.srv)-sum(a.dsc)-b.amount from pos_tdish a,pos_tmenu b 
