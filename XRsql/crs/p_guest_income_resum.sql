if exists (select 1 from sysobjects where name = 'p_guest_income_resum' and type = 'P')
   drop procedure p_guest_income_resum
;

create procedure p_guest_income_resum
as
begin 
	create table #lst
	(
	...
	from guest_income 
	where no in(select no from guest_income where sync<>'0' )
	group by no
	...
	update guest_income set sync = '0' where sync<>'0'
	return 0
end
;
