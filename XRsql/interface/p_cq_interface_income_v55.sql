drop procedure p_cq_interface_income_v55;
create proc p_cq_interface_income_v55
	@menu				char(10),	--对方单号
	@pc_id			char(4), 
	@shift			char(1),
	@empno			char(10), 
...
select @ref = accnt+'-'+guestid from guest where accnt = @accnt
select @ref = isnull(@ref,'')
select @deptno2 = isnull(tag1,''),@descript1 = isnull(descript1,'') from paymth where paycode = @paycode
if not exists(select 1 from paymth where paycode = @paycode)
	begin
...
--====================================================================
	...
	--如果食品有金额，折扣算到食品上，否则酒水有金额，折扣算到酒水上，否则香烟有金额，折扣算到香烟上
			...
			insert into pos_pay
				select @pos_menu,1,1,@descript1,@accnt,@roomno,'',@amount,'3','NR','',@empno,@bdate,@shift,getdate(),@ref, ''
		end
	if @pos_menu = '' 
		select @menu1 = @menu
	else
...
--====================================================================		
	if charindex(@deptno2, 'TOA#TOR') > 0 
		begin
		select @selemark = 'a' + @menu1 , @today = getdate()
		...
		exec @ret = p_gl_accnt_post_charge @selemark, 0, 0, '04', @pc_id, @shift, @empno, @accnt, @guestno, '', @pccode, @package, @amount, NULL, @today, NULL, 'IN', 'R', '', 'I', @msg out
		if @ret <> 0
			Goto gout
