...
if exists(select * from sysobjects where name = "p_gl_har_master_header4")
	drop proc p_gl_har_master_header4;

create proc p_gl_har_master_header4
	@pc_id				char(4),
	@mdi_id				integer,
	@roomno				char(5),
	@accnt				char(10),
...
//	from har_master where accnt = @accnt
//if @rmrate != @setrate
//	begin
...
select a.accnt, b.name, a.sta, roomno = space(5), c.starting_time, c.closing_time, c.to_roomno, c.to_accnt, c.name, c.pccodes, 
	...
	from har_master a, guest b, subaccnt c where a.accnt = @accnt and a.haccnt = b.no and a.accnt = c.accnt and c.subaccnt = @subaccnt and c.type = '5'
return 0;
