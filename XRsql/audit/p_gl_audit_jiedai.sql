if exists (select * from sysobjects where name ='p_gl_audit_jiedai' and type ='P')
	drop proc p_gl_audit_jiedai;
create proc p_gl_audit_jiedai
	@ret				integer		out, 
	@msg				varchar(70)	out
as
-----------------------------------------------------------------------------
...
while @@sqlstatus = 0
	...
	if @argcode >= '9'
		...
		-- 单笔的转AR账记入AR账发生额
		else if charindex(@deptno, @tor_str + @card_str) > 0
--			update dairep set debit = debit + @credit, sumcre = sumcre + @credit where class = @nt_netar
...
--		update dairep set debit = debit + @charge , sumcre = sumcre + @charge where class = @nt_netar
		exec p_gl_audit_jiedai_jiedai @nar, @accnt, @charge, 0, 0
--		select @deptno = deptno, @dept_od = (charindex(deptno, @p_deptnos) + 3) / 4 from pccode where pccode = @pccode
--		if @dept_od < 1
--			select @dept_od = 1
--		if not exists (select 1 from dairep where class = substring(@nt_netar, 1, 2) + @deptno)
--			insert dairep (class, descript, descript1, date) values (substring(@nt_netar, 1, 2) + @deptno, '  ' + substring(@p_deptnms, (@dept_od - 1) * 9 + 1, 8), '  ' + substring(@p_deptenms, (@dept_od - 1) * 9 + 1, 8), @bfdate)
--		update dairep set debit = debit + @charge, sumcre = sumcre + @charge where class = substring(@nt_netar, 1, 2) + @deptno
		end
	update dairep set sumcre = sumcre + @charge where class = @nt_credtl
	if charindex(@modu_id, @modu_ids) > 0 and not @tail is null
		-- 总台输入部分及电话计费需统计入底表借方 	
		begin	
		exec p_gl_audit_jiedai_jie @jierep, @tail, @roomno, @pccode, @tag, @charge, @charge1, @charge2, @charge3, @charge4, @charge5
--	新系统没有优惠，这一步不要了
--		if charindex(substring(@dsc_sttype, 2, 1), 'yY') > 0 and @servcode = 'H'
--			begin
--			update jierep set day09 = day09 - @charge where class = @jierep
--			-- deal with dairep about discount here when neccesary 
--			end
		end
	fetch gltemp_cursor into @accnt, @accntof, @pccode, @tag, @charge, @charge1, @charge2, @charge3, @charge4, @charge5, @credit, @modu_id, @roomno, @deptno, @jierep, @tail, @argcode 
	end
...
select a.pccode, a.paycode, a.descript1, a.creditd, b.deptno2, b.tail, b.deptno8, c.deptno from deptdai a, pccode b, pccode c, pos_pccode d 
...
--
open deptdai_cursor
fetch deptdai_cursor into @pccode, @paymth, @descript1, @creditd, @tag1, @tail, @dist, @deptno
while @@sqlstatus = 0
	begin
	if charindex(rtrim(@tag1), 'TOR#TOA#TOG#') != 0 or @tail in ('09', '10')
		begin
		fetch deptdai_cursor into @pccode, @paymth, @descript1, @creditd, @tag1, @tail, @dist, @deptno
		continue
		end
...
while @@sqlstatus = 0
	begin
	if charindex(@tag1, 'TOR#TOA#TOG#DSC#ENT') != 0
		begin
		fetch bosdai_cursor into @descript1, @creditd, @tag1, @tail
		continue
		end
...
open outtemp_cursor
fetch outtemp_cursor into @billno, @roomno, @accnt, @number, @paymth, @credit
...
if @isfstday ='T'
	...
	update dairep set 
		credit01m = credit01, credit02m = credit02, credit03m = credit03, 
...
if @isfstday = 'T'
	update dairep set last_blm = last_bl
