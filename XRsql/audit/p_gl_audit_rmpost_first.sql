...
declare c_rmpost cursor for select a.accnt, a.arr, a.rmposted, a.rmpoststa, isnull(b.name, a.roomno), isnull(b.street, ''), a.class
	from master a, guest b where a.sta = 'I' and a.rmpoststa != '1' and a.arr < @half_time and a.haccnt = b.no
	union select a.accnt, a.arr, a.rmposted, a.rmpoststa, isnull(b.name, ''), isnull(b.street, ''), a.class
	from ar_master a, guest b where a.sta = 'I' and a.rmpoststa != '1' and a.arr < @half_time and a.haccnt = b.no
	order by accnt
open c_rmpost
fetch c_rmpost into @accnt, @arr, @rmposted, @rmpoststa, @name, @fir, @class
while @@sqlstatus = 0
	begin
	if @arr < @whole_time
		select @w_or_h = 1
	...
	if @class = 'F' or exists(select 1 from rmpostpackage where pc_id = @pc_id and mdi_id = @mdi_id and accnt = @accnt and rule_calc like '1%')
		begin
		if @class != 'F'
			select @charge1 = 0, @charge2 = 0, @charge3 = 0, @charge4 = 0, @charge5 = 0
		insert rmpostbucket (accnt, roomno, src, class, name, fir, groupno, headname, type, market, ratecode, packages, paycode, cmscode, rmrate, qtrate, setrate, 
			charge1, charge2, charge3, charge4, charge5, package_c, rtreason, gstno, arr, dep, w_or_h, rmpostdate, logmark, empno, shift)
			select @accnt, roomno, src, class, @name, @fir, groupno, '', type, market, ratecode, packages, paycode, cmscode, @rmrate, @qtrate, @setrate, 
		...
		if @@rowcount = 0
			insert rmpostbucket (accnt, roomno, src, class, name, fir, groupno, headname, type, market, ratecode, packages, paycode, cmscode, rmrate, qtrate, setrate, 
				charge1, charge2, charge3, charge4, charge5, package_c, rtreason, gstno, arr, dep, w_or_h, rmpostdate, logmark, empno, shift)
				select @accnt, '', '', class, @name, @fir, '', '', '', '', '', '', paycode, '', @rmrate, @qtrate, @setrate, 
				@charge1, @charge2, @charge3, @charge4, @charge5, @package_c, '', 0, arr, dep, @w_or_h, @rmpostdate, logmark, '', ''
...
update rmpostbucket set headname = '['+rtrim(b.name)+']'
	from master a, guest b where rmpostbucket.rmpostdate = @rmpostdate and rmpostbucket.groupno = a.accnt and a.haccnt = b.no
update rmpostbucket set headname = '['+rtrim(b.name)+']', class = a.class
	from ar_master a, guest b where rmpostbucket.rmpostdate = @rmpostdate and rmpostbucket.groupno = a.accnt and a.haccnt = b.no
update rmpostbucket set headname = '[ 散客 ]' where rmpostdate = @rmpostdate and class = 'F' and groupno = ''
--update rmpostbucket set headname = '[ 长住客 ]', class = 'L' from master a, mktcode b
update rmpostbucket set headname = '[ 长住客 ]' from master a, mktcode b
...
update rmpostbucket set headname = '[ 应收账 ]' where rmpostdate = @rmpostdate and class = 'A'
