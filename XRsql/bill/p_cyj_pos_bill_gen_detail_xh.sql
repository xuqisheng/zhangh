...
--			select '', 0, '', '', '其中折扣[' + convert(char(2), convert(int, @dsc_rate * 100)) +'%]', 1, '',
--			round(isnull(sum( -1 * a.dsc), 0),2), 40, getdate(), 4000
--			from #dish a where charindex(rtrim(code), 'YZ') = 0
--  如果从dish取折扣，修改折扣率时，帐单打印会不平  cyj 03.12.19
		...
		insert #bill(menu, inumber, code, empno, name1, number, unit, amount,status, log_date, sort)
			select '', 0, '', '', a.tableno +'折扣'+'(' + isnull(rtrim(b.name2), ' ') + ')', 1, '',
	...
	insert #bill(menu, inumber, code, empno,name1, number, unit, amount, status, log_date, sort)
		select '', 0, '', '', '服务费[' + convert(char(2), convert(int, @srv_rate * 100)) +'%]', 1, '', round(isnull(sum(srv), 0),2), 20, getdate(), 2000
		from #dish a  where charindex(rtrim(code), 'YZ') = 0
