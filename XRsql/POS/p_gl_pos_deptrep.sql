...
update deptjie set feey = feey - feed  where date = @bdate
update deptjie set feed = 0,date = @bfdate,daymark = ' '
update deptdai set creditm = creditm - creditd  where date = @bdate
update deptdai set credity = credity - creditd  where date = @bdate
update deptdai set creditd = 0,date = @bfdate,daymark = ' '
commit tran

exec  p_cyj_pos_detail  '',	@bdate   --餐饮

-- 套菜报表分摊
exec p_cq_pos_detail_jie_link 'PCID',@bdate
...
insert #deptjie (pccode, shift, empno, code, feed) select b.pccode, a.shift, a.empno, 'ZZX', isnull(sum(a.charge),0)
	from gltemp a,pos_int_pccode b  where a.pccode = b.pos_pccode and a.shift = b.shift and charindex(modu_id, @modu_ids) > 0 
...
if exists(select 1 from sysoption where catalog ='pos' and item ='using_pos_int_pccode' and charindex(rtrim(value),'TtYy')>0 )
	...
	begin
	delete #araccnt where pccode not in(select pccode from pos_int_pccode where class = '2')
	update #araccnt set pccode = a.pccode from pos_int_pccode a where #araccnt.pccode = a.pos_pccode and #araccnt.shift = a.shift
	end
...
update deptjie set descript = a.descript from pos_namedef a
	where  a.code = deptjie.code
...
update deptdai set descript1 = pccode.deptno2 from pccode
	where rtrim(substring(deptdai.paycode,2,2)) = substring(pccode.deptno1,2,2) and pccode.pccode >'9'
...
if @isyfstday = 'T'
   update deptjie set feey = feed ,date = @bdate
else
   update deptjie set feey = feey + feed,date = @bdate
commit tran
declare c_deptjie cursor for select pccode,shift,empno,feed,feem,feey from deptjie where code='999'
open c_deptjie
fetch c_deptjie into @pccode,@shift,@empno,@feed,@feem,@feey
while @@sqlstatus = 0
   begin
	select @feedd = 0, @feemm = 0, @feeyy = 0
   select @feedd = feed, @feemm = feem, @feeyy = feey from deptjie where pccode=@pccode and shift = @shift and empno =@empno and code ='99A'
   ...
   if @feeyy = 0 or @feeyy is null
	  update deptjie set feey = 0 where pccode=@pccode and shift = @shift and empno =@empno and code ='99B'
   else
	  update deptjie set feey = round(@feey/@feeyy,2) where pccode=@pccode and shift = @shift and empno =@empno and code ='99B'
   fetch c_deptjie into @pccode,@shift,@empno,@feed,@feem,@feey
   end
close c_deptjie
...
if @isyfstday = 'T'
   update deptdai set credity = creditd ,date = @bdate
else
   update deptdai set credity = credity + creditd,date = @bdate
commit tran
