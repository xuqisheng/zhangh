...
if exists(select 1 from sysobjects where name='p_hry_audit_pdeptrep1' and type ='P')
	drop  proc p_hry_audit_pdeptrep1;
...
declare c_code cursor for select code from pos_namedef
	where  code <= '099' and deptno=@deptno order by code
open c_code
...
insert pdeptrep1 (pc_id, pccode, shift, descript2, descript)
	select @pc_id,b.pccode, '5', b.descript,  'Îå°à' from pos_pccode b where  b.deptno = @deptno


-- deptjie.code='ZZX' Ç°Ì¨ÈëÕË
if charindex(@daymark, 'mM') > 0
	declare c_ydeptjie cursor for select a.pccode, a.shift, a.code, a.feem
		from ydeptjie a, pos_pccode b
