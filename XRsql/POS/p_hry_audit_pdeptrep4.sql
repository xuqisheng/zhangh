...
if exists(select 1 from sysobjects where name = 'p_hry_audit_pdeptrep4')
	drop procedure p_hry_audit_pdeptrep4;
create proc p_hry_audit_pdeptrep4
	@pc_id		char(4),
	@date			datetime,
	@shift		char(1),
	@empno		char(10),
...
declare c_code cursor for select code from pos_namedef
	where (code <'6' or code = '99A') and deptno = @deptno 
union select '0ZZ'
order by code
open c_code
...
insert pdeptrep1 (pc_id, pccode, shift, descript2, descript)
	select @pc_id, b.pccode, a.code, b.descript, a.descript from basecode a,pos_pccode b where a.cat='pos_shift' and b.deptno = @deptno
--insert pdeptrep1 (pc_id, pccode, shift, descript2, descript)
...
insert pdeptrep1 (pc_id, pccode, shift, descript2, descript) select @pc_id, 'ZZZ', code, '合计', descript  from basecode where cat = 'pos_shift'
...
if charindex(@daymark, 'mM') > 0
	insert into #deptjie
		select a.pccode, a.shift, a.code, a.feem, a.feey from ydeptjie a, pos_pccode b
		where a.date = @date and a.empno = @empno and a.pccode = b.pccode  and b.deptno = @deptno
		and (a.code <= '200' or a.code = '99A') and shift <'9' 
else
	insert into #deptjie
		select a.pccode, a.shift, a.code, a.feed, a.feem from ydeptjie a, pos_pccode b
		where a.date = @date and a.empno = @empno and a.pccode = b.pccode  and b.deptno = @deptno
		and (a.code <= '200' or a.code = '99A') and shift <'9' 

insert into #deptjie
	select a.pccode,a.shift,'0ZZ',sum(a.fee),sum(a.feem) from #deptjie a where a.code <='099' 
	group by a.pccode,a.shift order by a.pccode,a.shift

declare c_ydeptjie cursor for select a.pccode, a.shift, a.code, a.fee, a.feem from #deptjie a
	order by a.pccode,a.shift, a.code

--select a.pccode, a.shift,a.code,a.fee from #deptjie a where a.code <>'0ZZ' and a.code <>'99A' and a.pccode ='001' and shift ='1'
--select a.pccode, a.shift, sum(a.fee), sum(a.feem) from #deptjie a where a.code <>'0ZZ' and a.code <>'99A' 
--	group by a.pccode,a.shift
--	order by a.pccode,a.shift

select @ccode = ''
open c_ydeptjie
fetch c_ydeptjie into @pccode, @cshift,@code, @fee, @feem
while @@sqlstatus = 0
	begin
	if @ccode <> @code
	...
	if @code <>'0ZZ' and @code <>'99A'
		begin
		update pdeptrep1 set vtl = vtl + @fee where pc_id = @pc_id and (pccode = @pccode or pccode = 'ZZZ')
			and (shift = @cshift or shift = '9' or shift = 'D' ) 
		update pdeptrep1 set vtl = vtl + @feem where pc_id = @pc_id and (pccode = @pccode or pccode = 'ZZZ') and shift = 'M'
		end
	fetch c_ydeptjie into @pccode, @cshift, @code, @fee, @feem
	end
close c_ydeptjie
