...
select @inumber = 1,@currentdt = @bdate
...
update #place_date set selected = convert(char(1),week) where datediff(dd,dt_date,@currentdt) = 0
--select * from #place_date order by dt_date,date_t
...
while @@sqlstatus = 0
	...
	if @week = 1
		begin
		select @check = @date_t
		select @logdate = @dt_date
		insert #date select @date_t,'','','','','','',@color,'','','','','','',@status,'','','','','','',@dt_date,getdate(),getdate(),getdate(),getdate(),getdate(),getdate(),'',@bdate,@inumber
		end
	if @week = 2
		update #date set date_2 = @date_t,color_2 = @color,status_2 = @status,dtdate_2 = @dt_date where dtdate_1 = @logdate
	if @week = 3
		update #date set date_3 = @date_t,color_3 = @color,status_3 = @status,dtdate_3 = @dt_date where dtdate_1 = @logdate
	if @week = 4
		update #date set date_4 = @date_t,color_4 = @color,status_4 = @status,dtdate_4 = @dt_date where dtdate_1 = @logdate
	if @week = 5
		update #date set date_5 = @date_t,color_5 = @color,status_5 = @status,dtdate_5 = @dt_date where dtdate_1 = @logdate
	if @week = 6
		update #date set date_6 = @date_t,color_6 = @color,status_6 = @status,dtdate_6 = @dt_date where dtdate_1 = @logdate
	if @week = 7
		update #date set date_7 = @date_t,color_7 = @color,status_7 = @status,dtdate_7 = @dt_date where dtdate_1 = @logdate
	if @dt_date = @currentdt
		update #date set selected = @selected where dtdate_1 = @logdate
