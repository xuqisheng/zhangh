drop  proc p_hry_audit_pdeptrep;
create proc p_hry_audit_pdeptrep
	@pc_id		char(4),
	@date			datetime,
	@shift		char(1),
	@empno		char(10),
...
insert pdeptrep (pc_id, jiedai, code, descript)
	select distinct @pc_id, 'A', code, descript from ydeptjie
	where date = @date and shift = @shift and empno = @empno and charindex(rtrim(pccode), @pccodes) > 0
if charindex(@daymark, 'mM') > 0
	declare c_ydeptjie cursor for select pccode, code, feem from ydeptjie
		where date = @date and shift = @shift and empno = @empno and charindex(rtrim(pccode), @pccodes) > 0
		order by pccode, code
else
	declare c_ydeptjie cursor for select pccode, code, feed from ydeptjie
		where date = @date and shift = @shift and empno = @empno and charindex(rtrim(pccode), @pccodes) > 0
		order by pccode, code
select @spccode = ''
open c_ydeptjie
...
insert pdeptrep (pc_id, jiedai, code, coden, descript)
	select distinct @pc_id, 'B', paycode, paytail, descript from ydeptdai
	where date = @date and shift = @shift and empno = @empno and charindex(rtrim(pccode), @pccodes) > 0
if charindex(@daymark, 'mM') > 0
	declare c_ydeptdai cursor for select pccode, paycode, paytail, creditm from ydeptdai
		where date = @date and shift = @shift and empno = @empno and charindex(rtrim(pccode), @pccodes) > 0
		order by pccode, paycode, paytail
else
	declare c_ydeptdai cursor for select pccode, paycode, paytail, creditd from ydeptdai
		where date = @date and shift = @shift and empno = @empno and charindex(rtrim(pccode), @pccodes) > 0
		order by pccode, paycode, paytail
select @spccode = ''
open c_ydeptdai
...
select @itemcnt = 0
declare c_pdeptrep cursor for select jiedai, code, coden from pdeptrep where pc_id = @pc_id order by jiedai, code, coden
open c_pdeptrep
fetch  c_pdeptrep into @jiedai, @code, @coden
while @@sqlstatus = 0
	begin
	select @itemcnt = @itemcnt + 1
	update pdeptrep set itemcnt = @itemcnt
		where pc_id = @pc_id and jiedai = @jiedai and code = @code and coden = @coden
	fetch  c_pdeptrep into @jiedai, @code, @coden
	end
close c_pdeptrep
deallocate cursor c_pdeptrep
update pdeptrep set itemcnt = itemcnt - (select count(1) from pdeptrep b where b.pc_id = @pc_id and b.jiedai = 'A')
	where pc_id = @pc_id and jiedai = 'B'
return 0
