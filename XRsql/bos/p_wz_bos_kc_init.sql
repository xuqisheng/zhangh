	...
	begin
		if @charge = 'T'
			insert bos_store(id,pccode,site,code)
				select isnull(@id,''),isnull(@pccode,''),isnull(@site,''),'init' 
			  union 
				select isnull(@id,''),isnull(@pccode,''),isnull(code,''),'init' from flrcode 
		else
			insert bos_store(id,pccode,site,code)
				select isnull(@id,''),isnull(@pccode,''),isnull(@site,''),'init' 
			  union 
				select isnull(@id,''),isnull(@pccode,''),isnull(roomno,''),'init' from rmsta 
