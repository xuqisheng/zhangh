...
//create unique index index1 on #bos_detail_temp(pccode,site,code,bdate)
		...
		select @id = id from bos_kcdate where datediff(dd,@begin_,begin_) <=0 and datediff(dd,@begin_,end_)>=0
...
--当前库存物品查询
if exists (select 1 from bos_store where id = (select id from bos_kcdate where datediff(dd,begin_,@begin_)>=0 and datediff(dd,end_,@begin_)<=0))
begin
