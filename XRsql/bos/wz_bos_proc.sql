...
// wz ---- bos相关的存储过程
//********************************************************************
//	p_wz_bos_kc_cx			实现当前和历史库存的查询
//	p_wz_bos_kc_cx_rep 	库存查询报表存储过程
//	p_wz_bos_kc_hdetail
//	p_wz_bos_kc_hdetail_head 
//
...
if exists (select 1 from sysobjects where name = 'p_wz_bos_kc_cx')
	drop proc p_wz_bos_kc_cx;

create Proc  p_wz_bos_kc_cx
	@modu			varchar(2)    	  , 
	@pccode0		varChar(3) = '%',
	@site0		varchar(5) = ''  ,
	@code0		varchar(8) = ''  ,
...
//create unique index index1 on #bos_detail_temp(pccode,site,code,bdate)
		...
		if datediff (day , @sysdate , @end_ ) > 0 
				select @ret = 1 ,@end_ = @sysdate
		
		if rtrim(@code0) = null 
		select @code0 = '%'
		if rtrim(@pccode0) = null 
		select @pccode0 = '%' 
		if rtrim(@site0) = null 
		select @site0 = '%'
		
		select @code0 = @code0 + '%' ,@pccode0 =  @pccode0 ,@site0 = @site0 +'%'
	...
	insert #bos_store_temp
		select a.pccode,a.site,a.code,a.number0,a.amount0,a.sale0,a.profit0,a.number1,a.amount1,a.sale1,a.profit1,a.number2,a.amount2,a.sale2,a.profit2,
             ...
             a.sale6,a.profit6,a.number7,a.amount7,a.sale7,a.profit7,a.number8,a.amount8,a.sale8,a.profit8,a.number9,a.amount9,a.sale9,a.profit9,a.price0,a.price1 
				 from bos_store a 
		       where  charindex(pccode,@pccode0)>0 
						and code like @code0	
						and site like @site0
...
//--插入临时表bos_detail数据
		if exists(select 1 from bos_hstore where id = @id)
			insert into #store_temp(spccode,ssite,scode,number0,samount0,sale0,profit0,price0,price1)  
				...
				where  id = @id
					and code like @code0  
					and charindex(pccode,@pccode0)>0 
					and site like @site0 
		else
			insert into #store_temp(spccode,ssite,scode,number0,samount0,sale0,profit0,price0,price1)  
		...
		where charindex(spccode,@pccode0)>0 and scode like @code0 and ssite like @site0
		...
		begin
			insert into #store_temp(spccode,ssite,scode,number0,samount0,sale0,profit0,price0,price1)  
				...
				where  scode like @code0  
					and charindex(spccode,@pccode0)>0 
		...
		select distinct a.ssite,a.scode,b.name,a.number9, a.price0,a.amount9,a.price1,a.sale9,a.profit9,a.number0,
			a.number1,a.number2,a.number3,a.number4,a.number5,a.number6,b.standent,b.unit,c.name,a.spccode
			from #bos_store_temp a ,bos_plu b, bos_provider c 
				where a.scode = b.code and a.spccode = b.pccode
					and b.provider = c.accnt 
...
select distinct a.ssite,a.scode,c.name,b.name,a.number0,a.number1 'ru',a.number2 'sun',a.number3 'pan',
	...
	begin
		...
		insert into #bos_detail_temp
			...
			and datediff(day,act_date ,@end_) >= 0 and code like @code0+'%'
					and pccode = @pccode0and site = @site0
			union
			select pccode,site,code,id,ii,flag,descript,folio,sfolio,fid,rsite,bdate,act_date,log_date,empno,number,
		...
		if @@rowcount = 0
			select @ret = 1 , @msg = '插入临时表bos_detail数据出错!'
...
// Reason: write procedure
//--------------------------------------------------------------------
// Modified By: mike wang		Date: 2003.03.18
//--------------------------------------------------------------------
//实现w_wz_bos_kc_hdeail0 里的dw_2的数据窗口 .
//====================================================================
...
select distinct c.site,c.name,a.pccode,a.code,a.name,a.standent,a.unit,
    a.provider,b.name,@begin_ 'begin',@end_ 'end',datediff(day,@begin_,@end_) 'lenth'
from bos_plu a, 
     bos_provider b, 
     bos_site c
     where a.pccode = @pccode0 and a.code = @code0 
        and a.provider = b.accnt and c.site = @site0  
...
// 
//====================================================================
create Proc  p_wz_bos_kc_hdetail0
	@pccode0		varChar(2),
	@site0		varchar(5),
	@code0		varchar(8),
	@begin_		DateTime,
...
if @ret = 0
	begin
		...
		insert into #bos_detail_temp
				select pccode,site,code,id,ii,flag,descript,folio,sfolio,fid,rsite,bdate,act_date,log_date,empno,number,
					amount0,amount,disc,profit,gnumber,gamount0,gamount,gprofit,price0,price1 from bos_detail
				where datediff(day,@begin_, bdate) >= 0
				and datediff(day,bdate ,@end_) >= 0 and code like @code0+'%'
						and pccode = @pccode0and site = @site0
			union
				select pccode,site,code,id,ii,flag,descript,folio,sfolio,fid,rsite,bdate,act_date,log_date,empno,number,
					amount0,amount,disc,profit,gnumber,gamount0,gamount,gprofit,price0,price1 from bos_hdetail
				where datediff(day,@begin_ , bdate) >= 0 and datediff(day, bdate,@end_) >= 0 and code like @code0+'%'
						and pccode = @pccode0 and site = @site0
				order by pccode,site,code,bdate
		...
		select ii,flag,bdate,act_date,descript,folio,sfolio,rsite,empno,
			number,amount,disc,gnumber,gamount,price1,pccode,site,code
		from #bos_detail_temp
		where  pccode = @pccode0 and  site = @site0
			and  code = @code0
		order by act_date
