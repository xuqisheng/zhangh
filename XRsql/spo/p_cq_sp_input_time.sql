...
//
/*-----------------------------------------------------------------------------*/
if exists(select 1 from sysobjects where name = 'p_cq_sp_input_time' and type = 'P')
	drop proc  p_cq_sp_input_time;

create proc p_cq_sp_input_time
	@menu			char(10),
	@empno		char(10),
	@inumber		integer	,	
	@begin_time	datetime,	          
...
//else
//	begin        
	select @code = b.plucode,@minutes = datediff(minute,stime,etime) from sp_plaav a, sp_place b
		where a.placecode = b.placecode and a.menu = @menu and a.inumber = @inumber
	if @@rowcount = 0
		select @ret = 2,@msg = "点菜单错误"
	else
		begin
		if @begin_date <> @begin_time
			select @begin_date = @begin_time
		select @timecode = timecode,@special = special,@plu_code = code,@price = price from pos_plu where  id = convert(integer,@code) //charindex(plucode,(select plucode from pos_plucode_link where pccode = @pccode))>0 and
		if @special <> 'S'
			select @amount0 = @price
		else
			begin
...
//cq
	...
	//                   select @begindate=datetime(date(getdate())+time(@bdate1)),@enddate=datetime(date(dateadd(dd,1,getdate()))+time(@edate1))
								select @begindate=convert(datetime,(substring(convert(char,dateadd(dd,@ii,@begin_date)),1,11)+''+substring(@bdate1,1,2)+':'+substring(@bdate1,3,2))),@enddate=convert(datetime,(substring(convert(char,dateadd(dd,@ii+1,@begin_date)),1,11)+''+substring(@edate1,1,2)+':'+substring(@edate1,3,2)))
								end
						 else
								begin
	//						   select @begindate=datetime(date(getdate())+time(@bdate1)),@enddate=datetime(date(getdate())+time(@edate1))
								select @begindate=convert(datetime,(substring(convert(char,dateadd(dd,@ii,@begin_date)),1,11)+''+substring(@bdate1,1,2)+':'+substring(@bdate1,3,2))),@enddate=convert(datetime,(substring(convert(char,dateadd(dd,@ii,@begin_date)),1,11)+''+substring(@edate1,1,2)+':'+substring(@edate1,3,2)))
								end
...
//				exec p_gl_pos_create_discount	@deptno,@pccode,@mode,@plu_code,@amount0,@dsc_rate,@result = @amount output
//
//				                            
//				exec p_gl_pos_create_serve		@deptno,@pccode,@mode,@plu_code,@amount0,@amount,@serve_rate,@result0 = @serve_charge0 output,@result = @serve_charge output
//
//				                            
//				exec p_gl_pos_create_tax		@deptno,@pccode,@mode,@plu_code,@amount0,@amount,@tax_rate,@result0 = @tax_charge0 output,@result = @tax_charge output
//				     
//
				update sp_plaav set stime =  @begin_date,etime = @end_time,amount = @amount0 where menu = @menu and inumber = @inumber       
//				update sp_dish set amount = @amount0, dsc = @amount0 - @amount, srv = @serve_charge, tax = @tax_charge, 
//					flag = stuff(flag, charindex('R',flag), 1,'s'),date1 = @begin_date,  date2 = @end_time	where menu = @menu and inumber = @inumber
//				                                 
//
//				            
//				update sp_menu set amount = amount + @amount + @serve_charge + @tax_charge
//				 where menu = @menu
//				select @charge = amount,@timestamp_new = timestamp from sp_menu where menu = @menu
				select @ret = 0,@msg = "成功"
				select @charge = @amount0
