drop  proc p_cq_sp_deptrep;
create proc p_cq_sp_deptrep
as
-----------------------------------------------------------------------------------*/
--
--		餐饮部门日报表: 夜审生成
...
--																									jjhotel cyj
--			定金统计，因为预定单倒入历史在前，所以别忘了关联sp_hreserver   cyj 2004/03/01
------------------------------------------------------------------------------------*/
declare
	@duringaudit		char(1),
	@bdate				datetime,
...
-- <逃帐追回>
if exists (select 1 from sp_tmenu where  menu not like convert(char(6), bdate, 12) +'%')
...
insert #deptdai (pccode,shift,empno,paycode,paytail,creditd)
	select a.pccode,b.shift,b.empno,'E' + substring(c.deptno1, 2, 2),'',sum(b.amount)
	from sp_tmenu a,#sp_pay b,pccode c
...
insert #deptdai (pccode,shift,empno,paycode,paytail,creditd)
	...
	from sp_tmenu a, sp_tpay b, master d
...
begin tran
delete ydeptjie where date = @bdate and pccode = '500'
delete ydeptdai where date = @bdate and pccode = '500'
if @subtotal <> 'T'
	begin
	if @ydeptrep = '9{{{'
		begin
