...
begin
	insert posdai (pc_id, pccode, paycode, paytail, creditd,descript)
		...
		from #sp_menu a, pos_detail_dai b,pccode c
		where a.menu = b.menu and a.paid = '1'
		and b.paycode = c.pccode and b.paycode = @pccod and substring(b.reason3, 1, 2) = '定' and (c.deptno8 = '' or c.deptno8 is null)
		group by a.pccode, c.deptno1
		fetch c_cur7 into @pccod,@payname
end
close c_cur7
...
declare c_detail_jie cursor for
	select pccode, menu, '0', amount0, tocode, '金额' from pos_detail_jie
	...
	union all select pccode, menu, '1', amount1+amount2, tocode, '百分比折扣' from pos_detail_jie
		where date = @date and shift like @shift and empno like @empno
		and charindex(pccode, @pccodes) > 0 and charindex(pccode, @limpcs) > 0
		and type = '0' and special <> 'T'


	union all select pccode, menu, '2', amount3, tocode, '特优码折扣' from pos_detail_jie
		where date = @date and shift like @shift and empno like @empno
		and charindex(pccode, @pccodes) > 0 and charindex(pccode, @limpcs) > 0	and special = 'T'
		and charindex(type,'486') = 0
	union all select pccode, menu, '4', amount3, tocode, '赠送' from pos_detail_jie
		where date = @date and shift like @shift and empno like @empno
		and charindex(pccode, @pccodes) > 0 and charindex(pccode, @limpcs) > 0	and special = 'N'
		and type = '4'
	union all select pccode, menu, '6', amount3, tocode, '全免' from pos_detail_jie
		where date = @date and shift like @shift and empno like @empno
		and charindex(pccode, @pccodes) > 0 and charindex(pccode, @limpcs) > 0	and special = 'N'
		and type = '6'
	union all select pccode, menu, '8', amount3, tocode, '单菜折扣' from pos_detail_jie
		where date = @date and shift like @shift and empno like @empno
		and charindex(pccode, @pccodes) > 0 and charindex(pccode, @limpcs) > 0	and special = 'N'
		and type = '8'
	union all select pccode, menu, '0', amount0, tocode, '金额' from pos_detail_jie
		where date = @date and shift like @shift and empno like @empno
		and charindex(pccode, @pccodes) > 0 and charindex(pccode, @limpcs) > 0
