drop procedure p_cq_tor_detail1;
create procedure p_cq_tor_detail1
			@bdate1		datetime,
			@bdate2		datetime	
...
declare c_dish cursor for
		...
		from sp_hdish c,sp_hmenu d where  c.sta <>'M' and c.menu = d.menu and d.bdate >= @bdate1 and d.bdate <= @bdate2 and charindex(rtrim(c.code),'XYZ') = 0
		and d.menu in (select a.menu from sp_hmenu a,sp_hpay b where a.sta = '3' and (a.setmodes = 'TOR' or b.paycode = 'TOR')
			 and a.bdate >= @bdate1 and a.bdate <= @bdate2
		...
		from pos_hdish c,pos_hmenu d where  c.sta <>'M' and c.menu = d.menu and d.bdate >= @bdate1 and d.bdate <= @bdate2 and charindex(rtrim(c.code),'YZ') = 0
