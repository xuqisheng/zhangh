...
while @@sqlstatus = 0
	...
	while @count > 0 and @csta = 'I' and @caccnt like 'F%'
		...
		if @ret = 0
			...
			select @to_accnt = '', @mode = 'T' + @roomno, @ref2 = substring(@roomno + space(5), 1, 5) + '(' + convert(char(10), @rmpostdate, 111) + ') 预收'
			if @operation = 'calculate'
				select @posting = @posting + @amount
			else
				exec @ret = p_gl_accnt_posting @selemark, @modu_id, @pc_id, @mdi_id, @shift, @empno, @caccnt, 0, @rmpccode, '', 
					@quantity, @amount, @charge1, @charge2, @charge3, @charge4, @charge5, @ratecode, @ref2, @rmpostdate, @rtreason, @mode, 'IRNY', 0, @to_accnt out, @msg out
			insert #account select @selemark, accnt, lastnumb, @amount, 0, ratecode, @ref2, @mode, @rmpostdate from master where accnt = isnull(rtrim(@to_accnt), @caccnt)
			-- 房租包价(将早餐, 娱乐等费用入帐)
			if @ret = 0
				begin
					...
					if @rule_calc like '1%'
						...
						if @operation = 'calculate'
							...
							exec @ret = p_gl_accnt_posting @selemark, @modu_id, @pc_id, @mdi_id, @shift, @empno, @caccnt, 0, @pccode, @argcode, 
								@quantity, @amount, @charge1, @charge2, @charge3, @charge4, @charge5, @ratecode, @ref2, @rmpostdate, @rtreason, @package, 'IRNY', @to_accnt out, null, @msg out
							insert #account select @selemark, accnt, lastnumb, 0, @amount, ratecode, @ref2, @package, @rmpostdate from master where accnt = isnull(rtrim(@to_accnt), @caccnt)
							end
						end
					if @ret != 0
						break
...
while @@sqlstatus = 0
	begin
	exec @ret = p_gl_accnt_posting @selemark, @modu_id, @pc_id, @mdi_id, @shift, @empno, @caccnt, 0, @pccode, @argcode, 
		@quantity, @amount, @charge1, @charge2, @charge3, @charge4, @charge5, @ratecode, @ref2, @rmpostdate, @rtreason, @mode, 'IRNY', 0, @to_accnt, @msg out
	fetch c_account into @selemark, @caccnt, @pccode, @argcode, @quantity, @amount, @charge1, @charge2, @charge3, @charge4, @charge5, @ratecode, @ref2, @mode, @rmpostdate, @rtreason, @package
	end
close c_account
...
if @roomno = '' and @accnt = ''				-- 所有
	delete #account where accnt not in (select a.accnt from accnt_set a where a.pc_id = @pc_id and a.mdi_id = @mdi_id and a.subaccnt = 0)
else if @accnt = ''								-- 指定房间
	delete #account where accnt not in (select a.accnt from accnt_set a where a.pc_id = @pc_id and a.mdi_id = @mdi_id and a.roomno = @roomno and a.subaccnt = 0)
else													-- 指定团体或账号
	delete #account where accnt not in (select a.accnt from accnt_set a where a.pc_id = @pc_id and a.mdi_id = @mdi_id and a.accnt = @accnt and a.subaccnt = 0)
insert gdsmsg select accnt+convert(char(10), number) from #account
