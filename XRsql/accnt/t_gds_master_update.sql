...
select @accnt=accnt, @opvod='F', @class=class, @setrate=setrate,@rmrate=rmrate, @gstno=gstno, @garr=arr, @arr=arr, @dep = dep,
		@amenities=amenities, @packages=packages, @market=market, @src=src, @srqs=srqs, @ratecode=ratecode,@new_sta = sta,@new_roomno = roomno ,
			@groupno = groupno
	from inserted
...
if @class='F' and (update(sta) or update(extra) or update(roomno) or update(haccnt))
	...
	if @new_sta = 'I' and @old_sta <> 'I'
        ...
        delete from phteleclos where roomno=@new_roomno and type='hrs' and changed='F'
        insert phteleclos(roomno,type,tag,wktime,changed,settime,chgtime,accnt)
    		select roomno, 'hrs', '',dep, 'F', getdate(), null, accnt from inserted a,mktcode b where a.market=b.code and b.flag='HRS' and a.accnt = (select min(accnt) from inserted)
		end
	...
	-- 换房
		...
		--考虑同住关系，如果有同住则不发ckou命令
			...
			begin
                ...
                delete from phteleclos where roomno=@old_roomno and type='hrs' and changed='F'
			end
...
begin
	...
	begin  -- 注意下面的次序
		...
		delete from phteleclos where roomno=@new_roomno and type='hrs' and (changed='F' or wktime=@dep)
		insert phteleclos(roomno,type,tag,wktime,changed,settime,chgtime,accnt)
        		select roomno, 'hrs', '',dep, 'F', getdate(), null, accnt from inserted a,mktcode b where a.market=b.code and b.flag='HRS' and a.accnt = (select min(accnt) from inserted)
	end
end
...
--add market
if update(market) and @sta='I'
begin
	if exists(select 1 from mktcode where code=@market and flag='HRS')
	begin
		delete from phteleclos where roomno=@new_roomno and type='hrs' and (changed='F' or wktime=@dep)
		insert phteleclos(roomno,type,tag,wktime,changed,settime,chgtime,accnt)
        		select roomno, 'hrs', '',dep, 'F', getdate(), null, accnt from inserted a,mktcode b where a.market=b.code and b.flag='HRS' and a.accnt = (select min(accnt) from inserted)
...
if update(groupno)
	begin
	if @groupno = '' or @groupno = null
		update master set exp_s3 = ''where accnt = @accnt
	else
		begin
		select @exp_s3 = haccnt from master where accnt = @groupno
		update master set exp_s3 = @exp_s3 where accnt = @accnt
		end
	end
---------------------------------------------------------------------------------
...
---------------------------------------------------------------------------------
--	Part. 9 master rmnum update---update rsvsrc_detail的quantity
---------------------------------------------------------------------------------
if update(rmnum)
begin
...
--	Part. 10 comsg, ref
---------------------------------------------------------------------------------
if update(comsg ) and exists(select 1 from hmaster where accnt=@accnt)
begin
	update hmaster set comsg=inserted.comsg from inserted where hmaster.accnt=inserted.accnt
end
if update(ref ) and exists(select 1 from hmaster where accnt=@accnt)
begin
	update hmaster set ref=inserted.ref from inserted where hmaster.accnt=inserted.accnt
end
;