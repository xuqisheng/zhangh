drop trigger t_zk_invoice_opdtl_delete;
create trigger t_zk_invoice_opdtl_delete
   on invoice_opdtl
   for delete as
...
select @id = id,	@inno = inno,@credit = credit,@remark = remark,@empno = empno,	@crtdate = crtdate,@pc_id = pc_id,@cby = cby,
			@changed = changed,@logmark = logmark from deleted
...
if rtrim(@accnt) <> null
	insert lgfl select 'arinvoice_d',@accnt,'',@msg + '单位名称:' + @unitname + ' 发票号:' + @inno + ' 金额:' + ltrim(rtrim(convert(char(20),@credit))) + ' 备注:' + @remark,@cby,@changed,''
