if exists(select * from sysobjects where type ="U" and name = "new_haccount")
   drop table new_haccount;

create table new_haccount
(
...
;
insert new_haccount select accnt, 1, number, inumber, modu_id, log_date, bdate, date, pccode + servcode, pccode, 1, charge, 
...
update new_haccount set billno = 'B' + substring(billno, 2, 9) where billno like '[OP]%';
update new_haccount set quantity=0 where pccode<'03';
update new_haccount set quantity=1 where pccode='02A' and mode like '[B,J,N]%';
update new_haccount set quantity=0.5 where pccode='02A' and mode like '[b,j,P]%';
//
update new_haccount set pccode = a.pos_item,argcode = substring(a.pos_item,1,2) from a_chgcod a where new_haccount.pccode = a.old_pccode;
update new_haccount set crradjt = '' where crradjt = 'NR';
update new_haccount set argcode = '98' where argcode in ('05', '06');
update new_haccount set argcode = '99' where argcode in ('03');
update new_haccount set pccode = a.pccode from pccode a where new_haccount.tag = a.deptno2;
// JJH
update new_haccount set pccode = '921' where pccode='920';
update new_haccount set pccode = '916' where pccode in ('917', '919');
update new_haccount set pccode = '918' where pccode='917';
update new_haccount set charge1 = charge where reason = '';
update new_haccount set charge2 = - charge where reason <> '';
