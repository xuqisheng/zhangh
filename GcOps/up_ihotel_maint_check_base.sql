...
BEGIN
	...
	INSERT INTO tmp_check_base SELECT NULL,'A',CONCAT('code_transaction:','  折扣类付款 reason error = ',CONCAT(a.code,'  ',a.descript,' ',a.category_code)) FROM code_transaction a WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND a.arrange_code>'9' AND a.cat_posting='ENT' AND a.is_need_reason<>'T';		
	INSERT INTO tmp_check_base SELECT NULL,'A',CONCAT('code_transaction:','  非折扣类付款 reason error = ',CONCAT(a.code,'  ',a.descript,' ',a.category_code)) FROM code_transaction a WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND a.arrange_code>'9' AND a.cat_posting<>'ENT' AND a.is_need_reason='T';		
	INSERT INTO tmp_check_base SELECT NULL,'A',CONCAT('code_transaction:','  非法的内部码 = ',CONCAT(a.code,'  ',a.descript,' ',a.cat_posting)) FROM code_transaction a WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND a.arrange_code>'9' AND NOT EXISTS(SELECT 1 FROM code_base b WHERE  b.hotel_id = arg_hotel_id AND b.hotel_group_id = arg_hotel_group_id AND b.parent_code='payment_flag' AND a.cat_posting=b.code);	
	INSERT INTO tmp_check_base SELECT NULL,'A',CONCAT('code_transaction:','  不能做为定金 = ',CONCAT(a.code,'  ',a.descript,' ',a.cat_posting)) FROM code_transaction a WHERE a.hotel_group_id = arg_hotel_group_id AND a.hotel_id = arg_hotel_id AND a.arrange_code>'9' AND (a.cat_posting='TA' OR a.cat_posting='TF') AND a.cat_bal<>'T';			
